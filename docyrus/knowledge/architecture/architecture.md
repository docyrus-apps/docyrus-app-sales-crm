...
Screens separate data from document: `/quotes/$quoteId` (Quote Detail, shared `RecordDetailLayout`) authors attributes + the Line Items pricing tab; `/quotes/$quoteId/build` (compose) renders pricing read-only and composes the document; `/quotes/new` opens the create wizard. The list is the `sales_order` route surfaced as "Teklifler"; its "Yeni Teklif" button opens the same wide modal wizard in-place. The wizard (`src/components/quotes/quote-create-wizard.tsx`) collects customer/deal basics, temporary `PricingEnginePanel` line items, and a template choice, then creates the `sales_order`, persists `sales_order_item` rows, stores the chosen template/doc metadata in localStorage, and navigates to Quote Detail. The editable pricing for an existing quote lives in `src/components/quotes/quote-line-items.tsx` (used editable in the Detail tab, read-only in the build preview).
