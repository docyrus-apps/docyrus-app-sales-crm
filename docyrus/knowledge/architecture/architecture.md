...
⚠️ **The Plate editor strips rich styling — so the editor no longer eagerly round-trips on mount.** `HtmlTemplateEditor`'s value-sync effect (`html-template-editor.tsx`) now sets the SSOT (`templateHtml`) **directly from `value`** without a Plate deserialize→serialize, so the **Code / Preview / PDF tabs render the body faithfully** (gradients, `<div>` layout, inline cell borders/widths, and color/size on `{{variables}}` are all preserved). Plate is loaded lazily **only when the Visual tab is opened** — and Visual DOES strip those on its round-trip, so avoid Visual for rich server-authored templates. This is what lets the app consume the existing rich backend templates as-authored. (An earlier approach instead embedded round-trip-safe templates in the app + a roundtrip test; both were removed when templates moved to the backend.) For reference, what Plate's Visual round-trip preserves vs. drops:
...
The editor **host stylesheet** still styles whatever body is rendered: `A4_PREVIEW_CSS` (`html-template-editor.tsx`, Preview) and `PDF_PAGE_CSS` (`lib/html-to-pdf.ts`, export) give `td,th` a `1px solid #e2e8f0` border (the editor is quote-only, so this is safe). The quote `data` is composed in `quote-build.tsx`: money via `formatCurrency v currency locale` (`documentLocale` = tr-TR default / en-US when UI=en), grand total spelled out via `numberToWordsTR` ("Yazıyla"), per-line amount = `net` (column sums to `totals.subtotal`). The header **PDF İndir** button compiles the rendered body + data with `createEditorTemplateEngine` and rasterizes via `lib/html-to-pdf` (client-side; no overwrite, no server PDF). `quote-templates.ts` imports `HandlebarsVariable` from `…/html-template-editor/types`, NOT the barrel `index` (the barrel pulls Plate + katex CSS and breaks Node imports).
