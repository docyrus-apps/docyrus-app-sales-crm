...
- The left pane is an internal `RecordAttributePanel` driven by `EditableRecordDetail`/`EditableRecordDetailField` (`src/components/docyrus/editable-record-detail.tsx`). It is headed by a `Rows3` "Attributes" label. Inline fields pass `editHint="progressive"` + `size="sm"`; a scoped `.crm-attribute-panel <style>` normalizes every value renderer to a uniform **13px** (`[data-slot="editable-value"]` plus descendants `inherit`) so the key–value list reads consistently despite renderers' native `text-sm`/`text-xs` (renderers keep their native size elsewhere). The pencil opens a separate **plain form** — `RecordEditForm` (`src/components/crm/record-edit-form.tsx`) — inside a `Dialog` ("edit all"): a `useForm` + `DynamicFormField`-per-field always-open form with a sticky Cancel/Save footer that diffs values on submit. It deliberately does **not** reuse the inline click-to-edit `EditableValue` cells: their `document.body` fixed-position portal editor detaches from its anchor when the dialog scrolls and the caret never engages, so the modal is a normal form instead (fixed 2026-07-08). `RecordEditForm` is keyed by `recordVersion` and mounted only while `editOpen` so it seeds fresh values; custom `fieldRenderers` render as self-saving fields inside it. Both surfaces call the page's `onInlineSave(changes, values)`.
- **Attribute visual consistency**: labels wrap (`break-words` + `leading-tight`, not truncate); value-renderer roots carry `min-w-0 max-w-full` (icons `shrink-0`, single-line text `block truncate`) so long values truncate/shrink gracefully. In the **inline panel** `EditableValue` renders active edit controls through a `document.body` portal, anchored to the clicked value row with fixed positioning (this is fine in the narrow non-scrolling pane, but is why the edit-all modal uses a plain form instead — see above); wide compound editors (phone/money/duration/currency) get a readable minimum width without resizing the left pane. Popover/select contents use a higher shared z layer (`PopoverContent`/`SelectContent` at `z-[140]`) so dropdowns stay above the portal editor, the divider, dialogs, and pane overflow.
