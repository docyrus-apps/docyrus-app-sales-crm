...
Studio HTML templates are seeded through the CLI/backend, but the browser does **not** call `/dev/html-templates` because that path is not exposed to the app runtime. `src/components/quotes/quote-templates.ts` is the runtime template catalog and each preset carries its seeded backend template UUID (`backendTemplateId`): `Standard` is default, `Executive` and `Simple` are also available. `sales_order` has two custom persistence fields: `quote_template_id` (selected seeded template UUID) and `quote_doc_json` (document title, validity, billing/contact notes, plus per-quote edited template HTML and the template id it belongs to). Template body edits are saved per quote in `quote_doc_json`, not as global backend template CRUD.

⚠️ **Quote template HTML must be Plate-round-trip-safe.** The `HtmlTemplateEditor` deserializes the template into Plate on mount and re-serializes it back to HTML — even for the Preview/PDF tabs — so the rendered/saved body is the Plate-serialized form, NOT the raw string. Plate DROPS most styling, which is why a gradient/`<div>`-styled template renders unstyled. The verified safe subset (replicated against the real pipeline in `src/components/quotes/quote-templates.roundtrip.test.ts` via `createPlateEditor` + the editor kits + `preprocessHbsHtml`/`serializePlateToHbs`):
- ✅ survive: `<h1>-<h3>` / `<p>` / `<hr>` / `<blockquote>` / `<table><tr><td|th>`, `{{#each}}`/`{{#if}}`/helpers, **per-CELL `background-color`**, `text-align` on an INNER `<p>`, and — on **LITERAL text only** — `<span style="color|background-color|font-size">` + bold/italic/underline marks.
- ❌ dropped: any style on `<div>`/`<tr>`/`<table>` (gradients, border-radius, flex), inline cell **borders**, cell **width**, AND **color/font-size wrapped around a `{{variable}}`** (a variable keeps only bold/italic/underline — never color or size).

So the 3 presets author accents as cell `background-color` + literal-colored spans; the big title is `<h1>{{quote.title}}</h1>` (size from the host `<h1>` rule, not a span); value cells use light fills (values render dark) while strong/dark fills sit behind WHITE LITERAL labels (table headers). Table borders are NOT inline — they come from the editor **host stylesheet**: `A4_PREVIEW_CSS` (`html-template-editor.tsx`) for Preview and `PDF_PAGE_CSS` (`lib/html-to-pdf.ts`) for export both give `td,th` a `1px solid #e2e8f0` border (the editor is quote-only, so this global change is safe). Money is rendered with `formatCurrency v currency locale` (locale from `quote-build.tsx`'s `documentLocale`, tr-TR default / en-US when the UI is English) and the grand total is also spelled out via `numberToWordsTR` ("Yazıyla"); per-line amount = `net` so the column sums to `totals.subtotal`. `quote-templates.ts` imports `HandlebarsVariable` from `…/html-template-editor/types`, NOT the barrel `index` (the barrel pulls Plate + katex CSS and breaks Node/vitest imports).
