...
- **Inline editing is live** (per-page `useUpdate*` mutations). Scalar fields (text/email/phone/url/textarea) are editable everywhere. Enum and relation/user fields are now wired editable across **all four** detail routes via `useEnumEntities` + company/contact/user/country option lists — the old "Lead fields stay read-only" and "`country`/`city` read-only" carve-outs are gone:
  ...
  - **`country` is per-page typed**: in Lead it is the plural `countries` field, a relation to the `base/country` collection. In **Company**, `country` + `city` are consolidated into one **Location** attribute (`LocationField`) — country is a searchable picker over the populated `country` reference enum (`useEnumEntities('country')`, the same source the create dialogs and Deal use, so the saved option id stays consistent with existing records), city is plain text; the separate `country`/`city` rows were merged into it. Deal keeps `country` as a (now searchable) enum select.
