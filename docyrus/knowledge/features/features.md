...
- **Create wizard** — `/quotes/new` renders `QuoteCreatePage` (`src/routes/quote-create.tsx`) which opens the same `QuoteCreateWizard` used by the list page. The full-size modal keeps a fixed near-viewport height across all steps and uses a centered connected stepper. Steps: Customer/basics (centered single-column compact form with searchable-scrollable customer picker, optional deal, title/valid-until), Pricing (temporary editable `PricingEnginePanel`; product rows must stay catalog pickers whenever `productCatalog` is provided, even while empty/loading, and products come from the safe product columns `id,product_code,unit_price,tax,Unit` because the live/generated `product` datasource has no `name` or `category` field), Template (centered stacked seeded quote templates from `src/components/quotes/quote-templates.ts`; `Quote Standard` selected by default, with `Quote Executive` and `Quote Compact` also available). Create writes `sales_order` with Draft status when available + totals + `quote_template_id` + `quote_doc_json`, writes `sales_order_item` rows from the pricing document, then routes to `/quotes/$id` Detail. Builder is no longer the create surface.
...
- **Build / Compose layout** — `QuoteBuild` split screen. Header: template picker (seeded quote templates from `src/components/quotes/quote-templates.ts`, each mapped to a backend template UUID) showing the selected template name even while closed, plus a Mail icon (opens `QuoteEmailDialog`, enabled once the quote exists). LEFT is a narrower fixed-width form column: scrollable cards top→bottom (Customer picker · Line items button → **near-full-page modal** with `QuoteLineItems` — note shadcn `DialogContent` defaults to `sm:max-w-lg`, so the modal must override at the same breakpoint, e.g. `w-[96vw] sm:max-w-[1400px] h-[92vh]`) · Quote details (title/valid-until/billing) · Content (intro/terms)), then a non-scrolling fixed footer with the Create/Update button (saving→success effect). RIGHT = the Docyrus `HtmlTemplateEditor` (Plate + Handlebars). Left fields + saved quote compose the editor's JSON `data` **live** (the editor syncs its `data` prop, html-template-editor.tsx ~L758). Tabs visual/code/data/preview/**PDF** — PDF is client-side via the editor's own `lib/html-to-pdf` (the standalone `quote-pdf.ts` was removed). Doc fields + selected template id persist on `sales_order`; edited template body is saved per quote in `quote_doc_json` on Update. See [[architecture#Quote Builder & PDF]].
...
- **Tabs** are config-driven (`RecordDetailTab[]`): Overview (3 `RecordKpiCard` highlights + a framed "recent activity" list capped at 2) · Activity · entity-specific relation tabs (Company Team/Deals/Leads, Contact Deals, Deal Contacts/Products/Quotes/Orders) · **Comments** (`CommentsPanel`, value `comments`) · **Notes** (`RecordNotesPanel`, value `notes`) · **Tasks** (`RecordTasksPanel`, value `tasks`) · Files (`FileAttachments`) · "not available yet" placeholders for Emails/Calls.
  - **Comments author** — `CommentsPanel` resolves the author from `created_by` **or** `created_by_user` (the API may expose either; `created_by` is sometimes just the raw id string), trying `name` → `firstname+lastname` → `email`, with avatar from `photo`/`avatar` (helpers `resolveAuthor`/`authorName`/`authorAvatar`). Falls back to `comments.unknownAuthor`. ⚠️ The old code read only `created_by.name` → every comment showed "Unknown".
  - **Files upload** — `FileAttachments` (`src/components/shared/file-attachments.tsx`, shared by all four detail routes + quote detail) renders the DiceUI `FileUpload` **with children** — a `FileUploadDropzone` + `FileUploadTrigger` ("Choose files" button) — controlled with `value={[]}` and `onValueChange` → immediate upload. ⚠️ The root `<FileUpload>` is a context provider that renders nothing on its own; the earlier `<FileUpload onDrop=… />` (no children, and `onDrop` is not a real prop) showed no upload UI at all.
