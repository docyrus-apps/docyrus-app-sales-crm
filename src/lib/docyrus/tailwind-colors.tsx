...
/* eslint-disable */
/**
 * Tailwind CSS v4 color palette — hex values + utility functions.
 * Used by: AvatarSelect, ColorFormField, theme generator.
 */
...
/** Tailwind v4 color hex values — all 22 families × 11 tones */
...
/**
 * Resolve a Tailwind color name (e.g. "indigo-500") to a usable CSS value.
 * Returns hex from the built-in map first, falls back to CSS variable.
 */
export function resolveColorHex(color: string): string {
  ...
  if (trimmed.startsWith('hsl(') || trimmed.startsWith('hsla(')) return trimmed;
  if (trimmed.startsWith('var(')) return trimmed;

  const match = trimmed.match(/^(.+)-(\d+)$/);

  if (match) {
    const hex = match[1] && match[2] ? TAILWIND_HEX[match[1]]?.[Number(match[2])] : undefined;
