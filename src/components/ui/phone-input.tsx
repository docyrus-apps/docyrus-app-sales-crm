              ...
              {countries.map((c) => (
                <CommandItem
                  ...
                  onSelect={() => {
                    ...
                    const oldCountry = countries.find((x) => x.code === country);

                    store.setState("country", c.code);

                    if (currentValue && oldCountry && currentValue.startsWith(oldCountry.dialCode)) {
                      let rest = currentValue.slice(oldCountry.dialCode.length);
                      rest = rest.replace(/^\s+/, "");
                      store.setState("value", rest ? `${c.dialCode} ${rest}` : c.dialCode);
                    } else if (!currentValue) {
                      store.setState("value", c.dialCode);
                    }
