...
import {
  ...
  type HandlebarsChatMessage,
} from './handlebars-editor-types'

export interface HandlebarsAIAssistantProps {
  config: HandlebarsAIAssistantConfig
  open: boolean
  onClose: () => void
  template: string
  input: string
...
/**
 * Left-side AI Assistant drawer mounted inside `HandlebarsEditor`. Renders a
 ...
 */
export function HandlebarsAIAssistant({
  config,
  open,
  onClose,
  template,
  input,
}: HandlebarsAIAssistantProps) {
  ...
  const [messages, setMessages] = useState<HandlebarsChatMessage[]>([])
  ...
  const send = useCallback(async () => {
    ...
    const userMessage: HandlebarsChatMessage = {
      id: nextId(),
      role: 'user',
      content: trimmed,
    }
  ...
  }, [draft, isSending, messages, config, template, input, nextId])
            ...
            {messages.length === 0 ? (
              <ConversationEmptyState
                ...
                description={
                  ...
                  'Ask for help writing or debugging your Handlebars template.'
                }
              />
            ) : (
