...
import { QueryBuilderDocyrus, type RuleGroupType, type FullField } from '@/components/docyrus/query-builder';
import {
  Field,
  FieldDescription,
  FieldError,
...
const isRuleGroupType = (value: unknown): value is RuleGroupType => typeof value === 'object'
  && value !== null
  && 'combinator' in value
  && 'rules' in value
...
const DEFAULT_FIELDS: FullField[] = [
  {
    name: 'field', label: 'Field', value: 'field', inputType: 'text'
  }
];
...
function QueryBuilderInput({
  ...
  const query = useMemo(() => parseQuery(field.state.value), [field.state.value]);

  const qbFields = useMemo<FullField[]>(
    () => enumOptions.length > 0 ? enumOptions.map(opt => ({
      name: opt.id, label: opt.name, value: opt.id, inputType: 'text'
    })) : DEFAULT_FIELDS,
    [enumOptions]
  );
  ...
  return (
    <Field data-invalid={isInvalid} className={className}>
      ...
      <QueryBuilderDocyrus
        ...
        variant="bordered" />
      <FieldDescription>
        Visual query builder. Define rules and groups to build filter expressions.
      </FieldDescription>
