...
function getNumberFormatOptions(
  ...
  return {
    ...
    currencyDisplay:
      style === 'currency' ? (format?.currencyDisplay ?? 'symbol') : undefined,
...
function normalizeFormatValue(
  ...
  if (
    ...
    (format.percentScale ?? 'whole') === 'whole'
  ) {
    return value / 100
  }
...
export function getComparisonViewModel(
  ...
  const tone =
    direction === 'neutral'
      ? 'neutral'
      : comparison.positiveDirection === 'down'
        ? direction === 'down'
          ? 'positive'
          : 'negative'
        : direction === 'up'
  ...
  const percentageChange =
    comparison.previousValue === 0
      ...
      : ((value - comparison.previousValue) /
          Math.abs(comparison.previousValue)) *
        100
  const percentageLabel =
    percentageChange === null ? null : formatComparisonPercent(percentageChange)
  const caption = [
    deltaLabel,
    percentageLabel ? `(${percentageLabel})` : null,
    `vs ${PERIOD_LABELS[comparison.period]}`,
  ]
    .filter(Boolean)
    .join(' ')
