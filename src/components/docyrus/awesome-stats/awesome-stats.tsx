        ...
        {menuItems.map(menuItem => (
          <button
            ...
            className={cn(
              ...
              menuItem.variant === 'destructive' && 'text-destructive hover:bg-destructive/10 hover:text-destructive'
            )}
            onClick={(event) => {
...
const AwesomeStats = forwardRef<HTMLDivElement, AwesomeStatsProps>(({
  ...
  const getMenuItemsForCard = (item: AwesomeStatItem): AwesomeStatsCardMenuItem[] => {
  ...
  const renderCard = (item: AwesomeStatItem) => {
    ...
    const hasBottomChart = item.miniChart && (item.miniChart.position ?? 'right') === 'bottom';
  ...
  return (
    <div
      ...
      {layout.type === 'tabs' ? (
        orderedItems.length === 1 && firstItem ? renderCard(firstItem) : (
          <AwesomeStatsTabs
            items={orderedItems}
            defaultTabId={layout.defaultTabId}
            renderItem={item => renderCard(item)}
            sortable={sortingEnabled}
            onItemsOrderChange={sortingEnabled ? onOrderedItemsChange : undefined} />
        )
      ) : null}
    </div>
  );
