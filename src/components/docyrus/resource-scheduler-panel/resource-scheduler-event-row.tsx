...
import {
  dateRangeToWidth, dateToPixel, pixelToDate, stackEvents
...
export function ResourceSchedulerEventRow({
  ...
  const computedHeight = Math.max(rowHeight, maxSubRows * (SUB_ROW_HEIGHT + SUB_ROW_GAP) + SUB_ROW_GAP * 2);
  ...
  return (
    <div
      ...
      <div className="pointer-events-none absolute inset-0 flex">
        {Array.from({ length: Math.ceil(totalTimelineWidth / activeConfig.unitWidth) }).map((_, i) => (
          <div
            key={i}
      ...
      {stacked.map(({ event, subRow }) => {
        ...
        const width = dateRangeToWidth(event.startDate, event.endDate, activeConfig);
