...
function getEndOfUnit(unit: TimeUnit, date: Date): Date {
  switch (unit) {
    case 'hour': return new Date(startOfHour(date).getTime() + 3_600_000 - 1);
...
function diffInUnit(unit: TimeUnit, a: Date, b: Date): number {
  switch (unit) {
    ...
    case 'year': return differenceInMonths(a, b) / 12;
  }
}
...
export function getDefaultDateRange(preset: TimelinePreset): { start: Date; end: Date } {
  const now = new Date();
  ...
  switch (preset.id) {
    ...
    case 'weeks-days':
      return { start: startOfWeek(now, { weekStartsOn: 1 }), end: addWeeks(startOfWeek(now, { weekStartsOn: 1 }), 2) };
...
export function stackEvents<T extends { startDate: Date; endDate: Date }>(
  ...
  for (const event of sorted) {
    ...
    while (subRow < subRowEndTimes.length && (subRowEndTimes[subRow] ?? 0) > event.startDate.getTime()) {
      subRow++;
    }
...
export {
  format, diffInUnit, getStartOfUnit, getEndOfUnit, addUnit
};