...
import {
  ...
  type ResourceSchedulerContextValue,
} from './resource-scheduler-context'
...
export function ResourceSchedulerPanel({
  ...
  const activePreset = useMemo(
    () => getPresetById(presets, activePresetId),
    [presets, activePresetId],
  )

  const defaultRange = useMemo(
    () => getDefaultDateRange(activePreset),
    [activePreset],
  )
  const [viewStart, setViewStart] = useState<Date>(
    startDateProp ?? defaultRange.start,
  )
  ...
  const handleNavigate = useCallback(
    ...
    [viewStart, viewEnd, activePreset.config, onDateRangeChange],
  )
  ...
  const cssVariables = useMemo<CSSProperties>(
    ...
    [rowHeight, resourcePanelWidth, activePreset.config.unitWidth],
  )
