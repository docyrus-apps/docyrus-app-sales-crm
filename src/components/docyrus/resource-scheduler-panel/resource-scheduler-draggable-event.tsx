...
export function ResourceSchedulerDraggableEvent({
  ...
  const handleMoveDragMove = useCallback(({ activatorEvent, delta }: { activatorEvent: Event; delta: { x: number; y: number } }) => {
    const durationMs = differenceInMilliseconds(prevEndRef.current, prevStartRef.current);
    ...
    const resourceId = getResourceIdAtY(windowY, timelineBodyRef, resources, rowHeight);

    if (resourceId) {
      targetResourceRef.current = resourceId;
      const resourceIndex = resources.findIndex(r => r.id === resourceId);
      const originalIndex = resources.findIndex(r => r.id === event.resourceId);
  ...
  const handleMoveDragEnd = useCallback(() => {
    ...
    setIsDragging(false);
    const newResourceId = targetResourceRef.current !== event.resourceId ? targetResourceRef.current : undefined;

    onEventMove?.(event, currentStartRef.current, currentEndRef.current, newResourceId);
  ...
  const handleLeftResizeMove = useCallback(({ delta }: { delta: { x: number } }) => {
  ...
  const handleRightResizeMove = useCallback(({ delta }: { delta: { x: number } }) => {
