...
import { type HandlebarsBlockHelper, type THandlebarsBlockOpenElement } from '../types';
...
const HELPER_COLORS: Record<string, string> = {
  ...
  unless: 'border-orange-300 bg-orange-50 text-orange-800 dark:border-orange-700 dark:bg-orange-900/30 dark:text-orange-300',
...
function resolveHelperDefinition(name: string, helpers: HandlebarsBlockHelper[]): HandlebarsBlockHelper {
...
export function HandlebarsBlockOpenElement(
  ...
  function handleSubmit() {
    ...
    editor.tf.setNodes(
      { expression: draftExpression.trim() },
      { at: path }
    );
    setOpen(false);
  }
