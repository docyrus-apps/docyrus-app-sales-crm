...
function buildPreviewSrcDoc(html: string): string {
  return `<!doctype html><html><head><meta charset="utf-8"><style>${A4_PREVIEW_CSS}</style></head><body><div class="page">${html}</div></body></html>`
}
...
export function HtmlTemplateEditor({
   ...
   */
  const [activeTab, setActiveTab] = useState<HtmlTemplateEditorTab>(defaultTab)
              ...
              <div className="min-h-0 flex-1 overflow-auto bg-muted/30 p-6">
                {preview?.error ? (
                  ...
                  <iframe
                    ...
                    srcDoc={buildPreviewSrcDoc(preview?.result ?? '')}
