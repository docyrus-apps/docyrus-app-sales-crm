...
import { useUiTranslation, type TranslateFn } from '@/hooks/docyrus/use-ui-translation';
...
const CURRENCY_KEY_HINT = /(price|amount|total|subtotal|cost|fee|tax|discount|paid|balance|net|gross)/i;
...
function inferAlignForFormat(format: ComputedColumnFormat): 'left' | 'right' | 'center' {
...
function formulaToExpression(formula: string): string {
  ...
  if (formula.startsWith('sumGrandTotal') || formula.startsWith('sumLineNets') || formula.startsWith('sumLineTaxes')) {
    /*
     * Old top-level helpers from the schema-driven era — surface as text
     * so user can see what it is and choose to rewrite.
     */
    return formula.replace(/items/g, '').replace(/sum\w+/, '').trim();
  }
...
const SIZE_VALUES: ComputedFontSize[] = [
  'xs',
  'sm',
  'base',
  'lg',
...
function formatOptions(t: TranslateFn): Array<{ value: ComputedColumnFormat; label: string }> {
  return FORMAT_VALUES.map(v => ({ value: v, label: formatLabel(t, v) }));
}

function alignOptions(t: TranslateFn): Array<{ value: 'left' | 'center' | 'right'; label: string }> {
  return ALIGN_VALUES.map(v => ({ value: v, label: alignLabel(t, v) }));
}

function weightOptions(t: TranslateFn): Array<{ value: ComputedFontWeight; label: string }> {
...
function formatLabel(t: TranslateFn, value: ComputedColumnFormat): string {
  switch (value) {
    case 'text': return t('ui.htmlTemplateEditor.formatText', 'Text');

    case 'number': return t('ui.htmlTemplateEditor.formatNumber', 'Number');

    case 'currency': return t('ui.htmlTemplateEditor.formatCurrency', 'Currency');

    case 'percent': return t('ui.htmlTemplateEditor.formatPercent', 'Percent');

    case 'date': return t('ui.htmlTemplateEditor.formatDate', 'Date');

    case 'computed': return t('ui.htmlTemplateEditor.formatComputed', 'Computed');

    default: return value;
  }
}

function alignLabel(t: TranslateFn, value: 'left' | 'center' | 'right'): string {
  switch (value) {
    case 'left': return t('ui.htmlTemplateEditor.alignLeft', 'Left');

    case 'center': return t('ui.htmlTemplateEditor.alignCenter', 'Center');

    case 'right': return t('ui.htmlTemplateEditor.alignRight', 'Right');
...
function weightLabel(t: TranslateFn, value: ComputedFontWeight): string {
  switch (value) {
    case 'normal': return t('ui.htmlTemplateEditor.weightNormal', 'Normal');

    case 'bold': return t('ui.htmlTemplateEditor.weightBold', 'Bold');
...
function aggregateLabel(t: TranslateFn, value: ComputedAggregate): string {
  switch (value) {
    case 'sum': return t('ui.htmlTemplateEditor.aggregateSum', 'Sum');

    case 'average': return t('ui.htmlTemplateEditor.aggregateAverage', 'Avg');

    case 'min': return t('ui.htmlTemplateEditor.aggregateMin', 'Min');

    case 'max': return t('ui.htmlTemplateEditor.aggregateMax', 'Max');

    case 'count': return t('ui.htmlTemplateEditor.aggregateCount', 'Count');
 ...
 */
const COLOR_SWATCHES: Array<{ value: string; nameKey: string; nameFallback: string }> = [
  { value: '', nameKey: 'ui.htmlTemplateEditor.colorNone', nameFallback: 'None' },
  { value: '#0f172a', nameKey: 'ui.htmlTemplateEditor.colorSlate', nameFallback: 'Slate' },
  { value: '#ef4444', nameKey: 'ui.htmlTemplateEditor.colorRed', nameFallback: 'Red' },
  { value: '#f59e0b', nameKey: 'ui.htmlTemplateEditor.colorAmber', nameFallback: 'Amber' },
  { value: '#10b981', nameKey: 'ui.htmlTemplateEditor.colorEmerald', nameFallback: 'Emerald' },
  { value: '#3b82f6', nameKey: 'ui.htmlTemplateEditor.colorBlue', nameFallback: 'Blue' },
  { value: '#a855f7', nameKey: 'ui.htmlTemplateEditor.colorViolet', nameFallback: 'Violet' },
  { value: '#f3f4f6', nameKey: 'ui.htmlTemplateEditor.colorGrayBg', nameFallback: 'Light gray bg' },
  { value: '#fef3c7', nameKey: 'ui.htmlTemplateEditor.colorAmberBg', nameFallback: 'Light amber bg' }
];
...
function buildInitialColumns(candidate: ArrayPathCandidate, sampleRow: Record<string, unknown>): FieldDraft[] {
...
export function TableConfigurationDialog({
  open, onOpenChange, data, onConfirm, initialConfig
  ...
  return (
    ...
    <Dialog open={open} onOpenChange={onOpenChange} modal={false}>
      <DialogContent
        className="!flex max-h-[90vh] max-w-[min(96vw,1200px)] flex-col gap-3 overflow-hidden p-0 sm:max-w-[min(96vw,1200px)]">
        {open && (
          <DialogBody
            data={data}
...
function DialogBody({
  data, initialConfig, onCancel, onConfirm
}: DialogBodyProps) {
  ...
  const [tableLabel, setTableLabel] = useState<string>(
    () => initialConfig?.label
      ?? (candidates[0] ? prettifyKey(lastSegment(candidates[0].path) || t('ui.htmlTemplateEditor.insertTable', 'Table')) : '')
  );
  /*
   ...
   */
  const [overrides, setOverrides] = useState<Record<string, Partial<FieldDraft>>>(
  ...
  const [footerRows, setFooterRows] = useState<FooterDraft[]>(
    () => (initialConfig?.footer ?? []).map(cfg => ({ ...cfg, _uid: makeFooterUid() }))
  );
  ...
  const fields = useMemo<FieldDraft[]>(() => {
    ...
    return base.map(f => ({ ...f, ...overrides[`${selectedPath}::${f.key}`] }));
  }, [
    selectedPath,
    candidates,
    data,
  ...
  const toggleAggregate = (fieldKey: string, aggregate: ComputedAggregate) => {
    setFooterRows((prev) => {
      const exists = prev.some(r => r.key === fieldKey && r.aggregate === aggregate);

      if (exists) {
        return prev.filter(r => !(r.key === fieldKey && r.aggregate === aggregate));
      }
  ...
  const numericFields = useMemo(
    () => fields.filter(f => f.visible !== false && (f.format === 'currency' || f.format === 'number' || f.format === 'percent')),
    [fields]
  );
  ...
  const updateSmartFooterLabel = (uid: string, nextLabel: string) => {
    setFooterRows(prev => prev.map(r => (r._uid === uid ? { ...r, label: nextLabel } : r)));
  };

  const updateSmartFooterExpression = (uid: string, nextExpression: string) => {
    setFooterRows(prev => prev.map(r => (r._uid === uid ? { ...r, formula: expressionToFormula(nextExpression) } : r)));
  };
    ...
    <>
      <DialogHeader className="shrink-0 border-b px-6 pt-6 pb-4">
        <DialogTitle className="flex items-center gap-2">
          ...
          {isEditMode ? t('ui.htmlTemplateEditor.dialogTitleEdit', 'Edit table') : t('ui.htmlTemplateEditor.dialogTitleCreate', 'Create table from data')}
        </DialogTitle>
        <DialogDescription>
          {t('ui.htmlTemplateEditor.dialogDescription', 'Pick an array path from the JSON data, choose which fields to display and configure column styling.')}
        </DialogDescription>
      </DialogHeader>
          ...
          <div className="grid grid-cols-[260px_1fr] gap-4">
            ...
            <div className="flex min-w-0 flex-col gap-3">
              <div className="flex items-center gap-2">
                <Label htmlFor="table-label" className="w-20 shrink-0 text-xs text-muted-foreground">
                ...
                <Input
                  ...
                  placeholder={t('ui.htmlTemplateEditor.dialogTitleFieldPlaceholder', 'Table title')}
                  className="h-8 text-sm" />
              </div>
              <Separator />
              {showSmartBanner && (
                <SmartFormulaList
                  availableKeys={fields.filter(f => f.visible !== false).map(f => f.key)}
      ...
      <DialogFooter className="shrink-0 border-t bg-background px-6 py-4">
        ...
        <Button onClick={handleConfirm} disabled={!canConfirm}>
          {isEditMode ? t('ui.htmlTemplateEditor.save', 'Save changes') : t('ui.htmlTemplateEditor.insert', 'Insert table')}
        </Button>
      </DialogFooter>
    </>
  );
    ...
    <div className="flex items-start gap-3 rounded-md border border-dashed bg-muted/30 px-4 py-6 text-sm text-muted-foreground">
      ...
      <div className="space-y-1">
        <div className="font-medium text-foreground">
          {t('ui.htmlTemplateEditor.dialogEmptyTitle', 'No suitable data found')}
        </div>
        <p>
          {t('ui.htmlTemplateEditor.dialogEmptyBody', 'Paste a JSON containing an object list into the Data tab to create a table.')}
          {' '}
      ...
      <ScrollArea className="h-72 rounded-md border bg-muted/20">
        <div className="flex flex-col gap-0.5 p-1.5">
          {candidates.map((c) => {
            ...
            const label = lastSegment(c.path) || t('ui.htmlTemplateEditor.dialogPathRootLabel', 'root');
...
function aggregatePillOptions(
  ...
  const values = isNumericFormat(format) ? AGGREGATE_VALUES_NUMERIC : AGGREGATE_VALUES_TEXT;
...
function defaultAggregateLabel(
  ...
  switch (aggregate) {
    case 'sum': return t('ui.htmlTemplateEditor.aggregateSumOfLabel', `${fieldLabel} — total`);

    case 'average': return t('ui.htmlTemplateEditor.aggregateAvgOfLabel', `${fieldLabel} — average`);

    case 'min': return t('ui.htmlTemplateEditor.aggregateMinOfLabel', `${fieldLabel} — min`);

    case 'max': return t('ui.htmlTemplateEditor.aggregateMaxOfLabel', `${fieldLabel} — max`);
...
function SmartFormulaList({
  availableKeys, savedFooters, onAdd, onLabelChange, onExpressionChange, onRemove
    ...
    <div className="flex flex-col gap-3 rounded-md border border-primary/30 bg-primary/5 px-3 py-3">
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-1.5 text-xs font-semibold text-primary">
          ...
          {t('ui.htmlTemplateEditor.smartTotalTitle', 'Smart total — write your own formula')}
        </div>
        <Button
          size="sm"
      ...
      {availableKeys.length > 0 && (
        <div className="flex flex-wrap items-center gap-1 text-[10px] text-muted-foreground">
          <span className="uppercase tracking-wider">
            {t('ui.htmlTemplateEditor.smartTotalAvailableFields', 'Available fields:')}
          </span>
          {availableKeys.map(k => (
            <code key={k} className="rounded bg-background px-1.5 py-0.5 font-mono text-[10px] text-primary">
              {k}
            </code>
          ))}
        </div>
      ...
      {savedFooters.length === 0 ? (
        <div className="rounded-md border border-dashed border-primary/30 bg-background px-3 py-4 text-center text-[11px] text-muted-foreground">
          {t('ui.htmlTemplateEditor.smartTotalEmptyHint', 'No totals yet. Click "+ Add total" to start — you can write expressions like')}
          {' '}
          <code className="rounded bg-muted px-1 text-[10px]">qty * unitPrice * (1 - discountPct/100)</code>
        </div>
      ) : (
        <div className="flex flex-col gap-2">
...
function SmartFormulaRow({
  row, onLabelChange, onExpressionChange, onRemove
  ...
  return (
    <div className="flex items-start gap-2 rounded-md border border-primary/20 bg-background p-2">
      <div className="flex min-w-0 flex-1 flex-col gap-2">
        <div className="flex flex-col gap-1">
          ...
          <Textarea
            ...
            placeholder={t('ui.htmlTemplateEditor.smartTotalLabelPlaceholder', 'e.g. Subtotal — any notation works')}
            rows={2}
      ...
      <button
        ...
        aria-label={t('ui.htmlTemplateEditor.smartTotalRemoveAria', 'Remove total')}
...
function FieldsEditor({
  fields, footers, onChange, onToggleAggregate
}: FieldsEditorProps) {
  const { t } = useUiTranslation();
  ...
  if (fields.length === 0) {
    return (
      <div className="rounded-md border border-dashed bg-muted/20 px-4 py-6 text-center text-xs text-muted-foreground">
        {t('ui.htmlTemplateEditor.fieldsEmpty', 'No fields found at the selected path.')}
      </div>
    );
  }
...
function FieldRow({
  field, activeAggregates, onChange, onToggleAggregate
}: FieldRowProps) {
  const { t } = useUiTranslation();
    ...
    <div
      ...
      <div className="flex items-center gap-2">
        ...
        <Input
          ...
          placeholder={t('ui.htmlTemplateEditor.fieldLabelPlaceholder', 'Header label')}
      ...
      <div className="flex flex-wrap items-center gap-x-3 gap-y-2">
        <LabeledControl label={t('ui.htmlTemplateEditor.fieldFormat', 'Type')}>
          <Select
            value={field.format}
            onValueChange={v => onChange(field.key, {
        ...
        <LabeledControl label={t('ui.htmlTemplateEditor.fieldAlign', 'Align')}>
          <SegmentedToggle
            ...
            options={alignOpts}
            onChange={v => onChange(field.key, { align: v as FieldDraft['align'] })} />
        </LabeledControl>

        <LabeledControl label={t('ui.htmlTemplateEditor.fieldWeight', 'Weight')}>
          <SegmentedToggle
            value={field.fontWeight ?? 'normal'}
            options={weightOpts}
            onChange={v => onChange(field.key, { fontWeight: v as ComputedFontWeight })} />
        </LabeledControl>

        <LabeledControl label={t('ui.htmlTemplateEditor.fieldSize', 'Size')}>
          <Select
            value={field.fontSize ?? 'sm'}
            onValueChange={v => onChange(field.key, { fontSize: v as ComputedFontSize })}>
        ...
        <LabeledControl label={t('ui.htmlTemplateEditor.fieldTextColor', 'Text color')}>
          <ColorPicker
            value={field.textColor ?? ''}
            onChange={v => onChange(field.key, { textColor: v || undefined })} />
        </LabeledControl>

        <LabeledControl label={t('ui.htmlTemplateEditor.fieldBgColor', 'Background')}>
          <ColorPicker
            value={field.backgroundColor ?? ''}
            onChange={v => onChange(field.key, { backgroundColor: v || undefined })} />
        </LabeledControl>

        <LabeledControl label={t('ui.htmlTemplateEditor.fieldAggregate', 'Footer total')}>
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    active ? 'border-primary bg-primary/10 text-primary' : 'border-border text-muted-foreground hover:bg-accent hover:text-accent-foreground'
                  )}
...
function ColorPicker({ value, onChange }: ColorPickerProps) {
  ...
  const swatchOptions = COLOR_SWATCHES.map(s => ({ ...s, label: t(s.nameKey, s.nameFallback) }));
  const currentLabel = swatchOptions.find(s => s.value === value)?.label ?? (value || noneLabel);
  ...
  return (
    <Select value={value || '__none__'} onValueChange={v => onChange(v === '__none__' ? '' : v)}>
      <SelectTrigger className="h-7 w-[112px] gap-2 px-2 text-xs">
        <span
          ...
          style={{
            ...
            backgroundImage: value ? undefined : 'linear-gradient(45deg, #cbd5e1 25%, transparent 25%), linear-gradient(-45deg, #cbd5e1 25%, transparent 25%), linear-gradient(45deg, transparent 75%, #cbd5e1 75%), linear-gradient(-45deg, transparent 75%, #cbd5e1 75%)',
      ...
      <SelectContent>
        ...
        {swatchOptions.filter(s => s.value).map(s => (
...
function readPath(data: unknown, path: string): Array<Record<string, unknown>> | undefined {
  if (!path) {
    return Array.isArray(data) && isPlainObject(data[0]) ? (data as Array<Record<string, unknown>>) : undefined;
  }
 ...
 */
function seedOverridesFromConfig(
  cfg: { dataPath: string; columns: ComputedColumnConfig[] }
