...
function filterItems(items: ComboboxItem[], query: string): ComboboxItem[] {
  ...
  return items.filter(
    item => item.id.toLowerCase().includes(q)
...
function ComboboxInner({ variables, helpers }: ComboboxInnerProps) {
  ...
  const allItems = useMemo(() => buildItems(variables, helpers), [variables, helpers]);
  ...
  useEffect(() => {
    buttonListRef.current[clampedActiveIndex]?.scrollIntoView({ block: 'nearest' });
  }, [clampedActiveIndex]);
          ...
          {items.map((item, index) => (
            <button
              ...
              className={cn(
                ...
                index === clampedActiveIndex ? 'bg-accent text-accent-foreground' : 'text-popover-foreground hover:bg-accent/60'
              )}
              onMouseDown={(e) => {
                e.preventDefault();
                selectItem(item);
              ...
              <span className="font-mono text-xs font-semibold">{item.preview}</span>
              {item.description && (
...
export function HbsTriggerCombobox({ variables, helpers }: HbsTriggerComboboxProps) {
