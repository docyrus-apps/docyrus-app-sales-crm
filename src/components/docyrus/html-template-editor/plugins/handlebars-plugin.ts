  ...
  parsers: {
    html: {
      deserializer: {
        ...
        rules: [
          { validNodeName: 'SPAN', validAttribute: { 'data-hbs': 'var' } },
        ],
        parse: ({ element }) => {
    ...
    html: {
      deserializer: {
        ...
        rules: [
          {
            validNodeName: 'SPAN',
            validAttribute: { 'data-hbs': 'block_open' },
          },
        ],
        parse: ({ element }) => ({
           ...
           */
          tableAnchored:
            element.getAttribute('data-hbs-table-anchored') === '1',
          children: [{ text: '' }],
        }),
      },
    },
    ...
    html: {
      deserializer: {
        ...
        rules: [
          {
            validNodeName: 'SPAN',
            validAttribute: { 'data-hbs': 'block_close' },
          },
        ],
        parse: ({ element }) => ({
  ...
  parsers: {
    html: {
      deserializer: {
        ...
        withoutChildren: true,
        rules: [
          { validNodeName: 'SPAN', validAttribute: { 'data-hbs': 'else' } },
        ],
        parse: ({ element }) => ({
          type: HBS_ELSE_KEY,
          tableAnchored:
...
export const HandlebarsNormalizerPlugin = createPlatePlugin({
  ...
  extendEditor: ({ editor }) => {
    ...
    const origNormalizeNode = editorAny.normalizeNode as
      | ((entry: any, options?: any) => void)
      | undefined
    const origToggleMark = editor.tf.toggleMark as (
      key: string,
      options?: { remove?: string | string[] },
    ) => void
    ...
    editor.tf.toggleMark = ((
      ...
      editor.tf.withoutNormalizing(() => {
        if (isOn) {
          ...
          if (options?.remove) {
            const removeKeys = Array.isArray(options.remove)
              ? options.remove
              : [options.remove]
