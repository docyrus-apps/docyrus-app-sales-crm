...
function inlineMarkStyles(node: TText): string[] {
  ...
  if (typeof n.backgroundColor === 'string') parts.push(`background-color:${n.backgroundColor}`);
  if (typeof n.fontSize === 'string' || typeof n.fontSize === 'number') {
    const v = String(n.fontSize);

    parts.push(`font-size:${/^\d+$/.test(v) ? `${v}px` : v}`);
  }
  if (typeof n.fontFamily === 'string') parts.push(`font-family:${n.fontFamily}`);
...
function blockStyles(el: TElement): string[] {
  ...
  if (typeof r.align === 'string' && r.align !== 'start' && r.align !== 'left') {
...
function cellStyles(el: TElement): string[] {
  ...
  const borders = r.borders as Record<string, { size?: number; style?: string; color?: string }> | undefined;
...
function fmtCell(
  ...
  switch (column.type) {
    ...
    case 'computed': {
      ...
      if (typeof next === 'number') return fmtMoney(next, ctx.currency, ctx.locale);
...
function adhocColumnStyle(cfg: ComputedColumnConfig): string {
  ...
  if (cfg.fontSize) parts.push(`font-size:${FONT_SIZE_PX[cfg.fontSize] ?? cfg.fontSize}`);
...
function renderAdhocCellExpression(cfg: ComputedColumnConfig, defaultCurrency: string): string {
  /*
...
function renderAdhocInTableFooter(
  ...
  const rowsHtml = rows
    .map((row) => {
      ...
      const aggregateExpr = buildAggregateExpression(row.aggregate, dataPath, row.key);
      const valueExpr = wrapAggregateForFormat(aggregateExpr, targetCol, row.aggregate, defaultCurrency);

      const cells = visibleColumns
        .map((col) => {
          const baseStyle: string[] = ['border-top:1px solid #cbd5e1', 'font-weight:600', 'padding:6px 8px'];

          if (col.align) baseStyle.push(`text-align:${col.align}`);
          if (row.backgroundColor) baseStyle.push(`background-color:${row.backgroundColor}`);
 ...
 */
function renderAdhocSmartTotals(rows: ComputedFooterConfig[], defaultCurrency: string): string {
  if (rows.length === 0) return '';
  ...
  const rowsHtml = sorted
    .map((row) => {
      ...
      const valueExpr = wrapAggregateForFormat(formula, synthetic, 'sum', defaultCurrency);
...
function renderAdhocFooter(
  ...
  return {
    tfoot: renderAdhocInTableFooter(inTable, visibleColumns, dataPath, defaultCurrency),
    afterTable: renderAdhocSmartTotals(smart, defaultCurrency)
  };
}

function renderAdhocComputedTable(el: TComputedTableElement, opts: SerializeOptions): string {
  ...
  const headerCells = columns
    .map((cfg) => {
      const style = adhocColumnStyle({ ...cfg, backgroundColor: cfg.backgroundColor ?? '#f8fafc' });
   ...
   */
  const { tfoot, afterTable } = renderAdhocFooter(el, columns, dataPath, opts.defaultCurrency);
  const hbPath = toHandlebarsPath(dataPath);
  const title = el.label?.trim();
  const caption = title ? `<caption style="caption-side:top;text-align:left;font-weight:600;font-size:12px;padding-bottom:4px;color:#0f172a">${escHtml(title)}</caption>` : '';
...
function renderComputedTable(
  ...
  const visibleColumns = schema.columns.filter((col) => {
    if (el.columnVisibility && col.key in el.columnVisibility) return el.columnVisibility[col.key] === true;
  ...
  const rowsHtml = rows
    .map((row, index) => {
      const ctx: ComputedColumnContext = {
        currency, locale, rows, index
      };
      const cells = visibleColumns
        .map((col) => {
          const value = col.type === 'computed' ? (col.compute ? col.compute(row, ctx) : '') : row[col.key];
  ...
  const footerHtml = (schema.footer ?? [])
    .map((footer) => {
      ...
      const formatted = footer.format ? footer.format(value, footerCtx) : typeof value === 'number' ? fmtMoney(value, currency, locale) : String(value);
  ...
  const totalsTable = footerHtml ? `<table style="margin-left:auto;width:360px;margin-top:8px">${footerHtml}</table>` : '';
...
function isAnchoredOpen(node: TNode): node is THandlebarsBlockOpenElement {
  return isElementOfType(node, HBS_BLOCK_OPEN_KEY)
...
function isAnchoredClose(node: TNode): node is THandlebarsBlockCloseElement {
  return isElementOfType(node, HBS_BLOCK_CLOSE_KEY)
...
function flattenAnchoredChipsFromParagraphs(children: TNode[]): TNode[] {
  ...
  for (const child of children) {
    ...
    const flushSegment = () => {
      const hasMeaningful = segment.some(c => !isTextLeaf(c) || c.text.trim().length > 0);
    ...
    for (const grandchild of el.children as TNode[]) {
      if (!isTextLeaf(grandchild) && (isAnchoredOpen(grandchild) || isAnchoredClose(grandchild))) {
        flushSegment();
...
function serializeChildren(
  ...
  while (i < children.length) {
    ...
    if (isAnchoredOpen(child)) {
      ...
      if (jTable !== -1 && isElementOfType(children[jTable] as TNode, 'table')) {
        ...
        if (kClose !== -1) {
          ...
          if (isAnchoredClose(closer) && closer.helper === child.helper) {
            ...
            if (trs.length >= 2) {
              ...
              const bodyTrsHtml = trs.slice(1).map(tr => serializeNode(tr as TNode, schemaMap, opts)).join('');
...
function serializeNode(
  ...
  if (typeof (node as TText).text === 'string') return serializeText(node as TText);
  ...
  switch (el.type) {
    ...
    case COMPUTED_TABLE_KEY:
      return serializeComputedTable(el as TComputedTableElement, schemaMap, opts);
    ...
    case 'th': {
      const r = el as Record<string, unknown>;
      const cs = typeof r.colSpan === 'number' && r.colSpan > 1 ? ` colspan="${r.colSpan}"` : '';
      const rs = typeof r.rowSpan === 'number' && r.rowSpan > 1 ? ` rowspan="${r.rowSpan}"` : '';
    ...
    case 'callout': {
      ...
      const bg = v === 'warning' ? '#fef3c7' : v === 'error' ? '#fee2e2' : '#eff6ff';
      const bd = v === 'warning' ? '#f59e0b' : v === 'error' ? '#ef4444' : '#3b82f6';
...
export function serializePlateToHbs(
  ...
  const opts: SerializeOptions = {
    defaultCurrency: options?.defaultCurrency ?? DEFAULT_SERIALIZE_OPTIONS.defaultCurrency,
