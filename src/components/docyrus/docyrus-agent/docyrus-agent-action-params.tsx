  ...
  return (
    <div className={cn('flex flex-col gap-4', className)}>
      ...
      <div className="space-y-1.5">
        <Label className="text-sm">{t('ui.agent.customPrompt', 'Custom prompt')}</Label>
        <Textarea
          className="min-h-16 resize-none"
          onChange={handleCustomPromptChange}
          placeholder={t('ui.agent.customPromptPlaceholder', 'Add additional instructions...')}
          value={customPrompt} />
      </div>

      <Button className="w-full" onClick={handleExecute} type="button">
        {t('ui.agent.execute', 'Execute')}
      </Button>
    </div>
  );
};