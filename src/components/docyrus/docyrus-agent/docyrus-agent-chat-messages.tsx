      ...
      >
        ...
        {isAssistant && showMessageActions && (
          <MessageActions className="mt-1 shrink-0 opacity-0 transition-opacity group-hover:opacity-100">
            <MessageAction
              onClick={handleCopy}
              tooltip={t('ui.agent.copyMessage', 'Copy message')}
            >
...
export const DocyrusAgentChatMessages = ({
  className,
}: DocyrusAgentChatMessagesProps) => {
  const { t } = useUiTranslation()
  const {
    messages,
    suggestions,
