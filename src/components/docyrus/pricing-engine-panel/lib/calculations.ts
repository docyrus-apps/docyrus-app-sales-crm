...
export function bankersRound(
  value: number,
  decimals: number = FINANCIAL_PRECISION,
): number {
...
export function calculateLineItem(
  ...
  config: Pick<
    IPricingConfig,
    'discountBeforeVat' | 'enableVat' | 'enableLineDiscount'
  >,
): ILineItemCalculated {
  ...
  const netAfterDiscount = bankersRound(
    lineSubtotal - bankersRound((lineSubtotal * discountPercent) / 100),
  )
...
export function calculateTotals(
  ...
  config: Pick<
    IPricingConfig,
    'enableVat' | 'enableGlobalDiscount' | 'enableAdjustment'
  >,
): ITotals {
  const subtotal = bankersRound(
    rows.reduce((sum, r) => sum + r.lineSubtotal, 0),
  )
  const lineDiscountTotal = bankersRound(
    rows.reduce((sum, r) => sum + r.discountAmount, 0),
  )

  const effectiveGlobalPercent = config.enableGlobalDiscount
    ? globalDiscountPercent
    : 0
  const afterLineDiscounts = bankersRound(subtotal - lineDiscountTotal)
  const globalDiscountAmount = bankersRound(
    (afterLineDiscounts * effectiveGlobalPercent) / 100,
  )
  ...
  if (effectiveGlobalPercent > 0 && afterLineDiscounts > 0) {
    ...
    vatTotal = bankersRound(
      rows.reduce(
        (sum, r) => sum + bankersRound(r.vatAmount * globalFactor),
        0,
      ),
    )
  } else {
    vatTotal = config.enableVat
      ? bankersRound(rows.reduce((sum, r) => sum + r.vatAmount, 0))
      : 0
  }
