...
function buildCategoryTree(items: ICategoryCatalogItem[]): ICategoryCatalogItem[] {
  ...
  for (const item of items) {
    const node = map.get(item.id) as ICategoryCatalogItem;
...
function CategoryTreeNode({
  node, depth, selectedId, expandedIds, searchQuery, onSelect, onToggle
...
export function PricingCategoryCell({ lineId, category, categoryId }: PricingCategoryCellProps) {
  const {
    updateLineItem, setLineItemCategory, onCategorySelect,
    categoryCatalog, readOnly, locale
  } = usePricingEngine();
  ...
  const handleSelect = useCallback((cat: ICategoryCatalogItem) => {
  ...
  const effectiveExpandedIds = useMemo(() => {
    ...
    return new Set([...expandedIds, ...getExpandedIdsForSearch(tree, lowerQuery)]);
  }, [expandedIds, lowerQuery, tree]);

  if (categoryCatalog && categoryCatalog.length > 0) {
    const selectedName = categoryId ? categoryCatalog.find(c => c.id === categoryId)?.name : undefined;
      ...
      <Popover
        ...
        <PopoverTrigger asChild disabled={readOnly}>
          <Button
            ...
            role="combobox"
            aria-expanded={open}
            className="h-8 w-30 justify-between px-2 font-normal shadow-none">
  ...
  return (
    <div className="flex items-center gap-1">
      <Input
        ...
        className="h-8 w-25 border-0 bg-transparent px-1 shadow-none focus-visible:ring-1"
        disabled={readOnly} />
