...
import {
  addLineItemToList,
  createLineItem,
  duplicateLineItemInList,
  removeLineItemFromList,
  reorderLineItemList,
  updateLineItemInList,
...
interface IPricingEngineContext {
  ...
  onProductSelect?: (
    lineId: string,
    callback: (product: IProductCatalogItem) => void,
  ) => void
  onCategorySelect?: (
    lineId: string,
    callback: (category: ICategoryCatalogItem) => void,
  ) => void
}
...
export function PricingEngineProvider({
  ...
  const [currency, setCurrencyState] =
    useState<ICurrencyConfig>(initialCurrency)
  ...
  useEffect(() => {
    ...
    if (controlledValue.globalDiscountPercent !== undefined)
      setGlobalDiscountPercentState(controlledValue.globalDiscountPercent)
    if (controlledValue.adjustment !== undefined)
      setAdjustmentState(controlledValue.adjustment)
    if (controlledValue.currency)
      setCurrencyState((prev) => ({ ...prev, ...controlledValue.currency }))
    if (controlledValue.config)
      setConfigState((prev) => ({ ...prev, ...controlledValue.config }))
    if (controlledValue.description !== undefined)
      setDescriptionState(controlledValue.description)
    if (controlledValue.termsAndConditions !== undefined)
      setTermsAndConditionsState(controlledValue.termsAndConditions)
  ...
  const totals = useMemo(
    ...
    [lineItemRows, globalDiscountPercent, adjustment, config],
  )
  ...
  const buildDocumentData = useCallback(
  ...
  const addLineItem = useCallback(
    (overrides?: Partial<ILineItem>) => {
      ...
      setLineItems((prev) =>
        addLineItemToList(prev, createLineItem(config, overrides)),
      )
    },
    [config],
  )
  ...
  const updateLineItem = useCallback(
    (id: string, updates: Partial<ILineItem>) => {
      isInternalChange.current = true
  ...
  const setLineItemFromProduct = useCallback(
    (lineId: string, product: IProductCatalogItem) => {
      isInternalChange.current = true
      setLineItems((prev) =>
        updateLineItemInList(prev, lineId, {
  ...
  const setLineItemCategory = useCallback(
    (lineId: string, cat: ICategoryCatalogItem) => {
      isInternalChange.current = true
      setLineItems((prev) =>
        updateLineItemInList(prev, lineId, {
          categoryId: cat.id,
...
export function usePricingEngine(): IPricingEngineContext {
  ...
  if (!context) {
    throw new Error(
      'usePricingEngine must be used within a PricingEngineProvider.',
    )
  }
