...
const pricingEnginePanelVariants = cva(
  'w-full border rounded-xl bg-card text-card-foreground',
  {
    variants: {
      variant: {
        default: '',
...
export interface PricingEnginePanelProps extends Omit<
  ComponentProps<'div'>,
...
const PricingEnginePanel = forwardRef<HTMLDivElement, PricingEnginePanelProps>(
  (
    ...
    const currencyOverrides: Partial<ICurrencyConfig> | undefined =
      defaultCurrency !== 'USD' ? { code: defaultCurrency } : undefined
    ...
    return (
      ...
      >
        <div
          ref={ref}
          className={cn(
            pricingEnginePanelVariants({ variant, size }),
            className,
          )}
          {...props}
        >
