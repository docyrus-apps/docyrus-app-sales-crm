import {
  type PivotGridRenderedColumnKind,
...
export function buildPivotGridCsv(
...
function buildHeaderMatrix(
  ...
  const headerMatrix = Array.from({ length: snapshot.headerDepth }, () =>
    Array.from(
      {
  ...
  snapshot.valueColumns.forEach((column, columnIndex) => {
    column.headerPath.forEach((label, level) => {
      ;(headerMatrix[level] as string[])[
        snapshot.rowHeaderLabels.length + columnIndex
      ] = label
    })
  })
...
export async function exportPivotGridExcel(
  ...
  for (
    let columnIndex = 0;
    columnIndex < snapshot.rowHeaderLabels.length;
    columnIndex += 1
  ) {
  ...
  for (let level = 0; level < snapshot.headerDepth; level += 1) {
    ...
    for (
      let columnIndex = valueColumnOffset + 1;
      columnIndex <= levelRow.length;
      columnIndex += 1
    ) {
  ...
  worksheet.columns = [
    ...snapshot.rowHeaderWidths.map((width) => ({
      width: Math.max(12, width / 8),
    })),
    ...snapshot.valueColumns.map((column) => ({
      width: Math.max(12, column.width / 8),
    })),
  ]
  ...
  worksheet.eachRow((row, rowNumber) => {
    row.eachCell((cell, columnNumber) => {
      ...
      const valueColumn = isValueColumn
        ? snapshot.valueColumns[
            columnNumber - snapshot.rowHeaderLabels.length - 1
          ]
        : null
      ...
      if (
        valueColumn?.kind === 'grand-total' &&
        dataRow.kind !== 'grand-total'
      ) {
        cell.fill = {
          type: 'pattern',
          pattern: 'solid',
