...
function assertDimensionCount<TData>(
  ...
  if (dimensions.length < 1 || dimensions.length > 3) {
    throw new Error(
      `PivotGrid requires between 1 and 3 ${axis} dimensions.`
    );
  }
}
...
function buildAxisTree<TData>(params: {
  ...
  const nodesById = new Map<string, PivotAxisValueNode<TData>>([[root.id, root]]);
  ...
  data.forEach((row, rowIndex) => {
    ...
    dimensions.forEach((dimension, dimensionIndex) => {
      ...
      if (!child) {
        const pathKeys = [...current.pathKeys, `${dimension.id}:${serializedValue}`];
        const pathLabels = [...current.pathLabels, formatDimensionValue(dimension, rawValue)];
...
function sortAxisNode<TData>(
  ...
  if (dimension) {
    node.children.sort((leftNode, rightNode) => {
      ...
      return leftNode.label.localeCompare(
        rightNode.label,
        undefined,
...
export function buildPivotGridStructure<TData>(params: {
  ...
  const rawRowIds = data.map((row, index) => getRowId?.(row, index) ?? String(index));
  ...
  data.forEach((row, rowIndex) => {
    ...
    const rowAggregateIds = [createLeafAggregateId('row', rowLeafNode.id), ...(rowTree.pathNodeIdsByIndex[rowIndex] ?? []).map(nodeId => createSubtotalAggregateId('row', nodeId)), createGrandTotalAggregateId('row')];
...
function createPathSegments<TData>(params: {
  ...
  if (kind === 'leaf') {
    node.pathLabels.forEach((label, index) => {
      ...
      const currentNode = currentNodeId ? structure.columnTree.nodesById.get(currentNodeId) : undefined;
      ...
      segments[index] = {
        ...
        isExpanded: currentNodeId ? expandedColumnIds[currentNodeId] !== false : false
      };
    });
...
function createLeafColumnsForMeasureSet<TData>(params: {
  ...
  const pathLabels = node ? kind === 'leaf' ? [...node.pathLabels] : [...node.pathLabels, SUBTOTAL_LABEL] : [GRAND_TOTAL_LABEL];

  return structure.measures.map((measure) => {
    const baseId = node ? kind === 'leaf' ? createLeafAggregateId('column', node.id) : createSubtotalAggregateId('column', node.id) : createGrandTotalAggregateId('column');
    ...
    return {
      ...
      measure,
      kind,
      headerPath: structure.measures.length > 1 ? [...baseHeaderPath, measure.label] : baseHeaderPath,
      headerPathSegments: structure.measures.length > 1 ? [
        ...baseSegments,
        {
          key: `${id}:measure`,
          label: measure.label
...
function buildRenderedCell<TData>(params: {
  ...
  return {
    ...
    isTotal:
      row.kind === 'grand-total' || column.kind === 'grand-total',
    isGrandTotal:
      row.kind === 'grand-total' && column.kind === 'grand-total'
  };
}
