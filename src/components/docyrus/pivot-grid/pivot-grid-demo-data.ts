...
export function createPivotGridDemoRows(
  ...
  for (const regionEntry of REGIONS.slice(0, regions)) {
    for (const countryEntry of regionEntry.countries.slice(
      0,
      countriesPerRegion,
    )) {
        ...
        for (const year of YEARS.slice(0, years)) {
          for (const quarter of QUARTERS.slice(0, quarters)) {
            for (const channel of CHANNELS.slice(0, channels)) {
              ...
              const revenue =
                base *
                (channel === 'Direct' ? 5.8 : channel === 'Partner' ? 4.9 : 4.2)
...
export const pivotGridDemoRowDimensions: Array<
  PivotGridDimension<PivotGridDemoRow>
> = [
  {
    id: 'region',
    label: 'Region',
...
export const pivotGridDemoColumnDimensions: Array<
  PivotGridDimension<PivotGridDemoRow>
> = [
  {
    id: 'year',
