...
const EMPTY_DRILLDOWN_COLUMNS: Array<
  DataGridColumnDef<Record<string, string>>
> = [
  {
    id: '__empty__',
    accessorFn: () => 'No rows',
...
function getMergedHeaderPinningStyle<TData>(params: {
  ...
  return {
    ...
    opacity: 0.98,
    boxShadow:
      pinned === 'left' && lastColumn.getIsLastColumn('left')
        ? '-4px 0 4px -4px var(--border) inset'
        : pinned === 'right' && firstColumn.getIsFirstColumn('right')
          ? '4px 0 4px -4px var(--border) inset'
          : undefined,
  }
}
...
function getDrilldownColumns<TData>(
  ...
  if (controller.drilldownColumns && controller.drilldownColumns.length > 0) {
    return controller.drilldownColumns as Array<
      DataGridColumnDef<TData | Record<string, string>>
    >
  }

  if (rows.length === 0) {
    return EMPTY_DRILLDOWN_COLUMNS as Array<
      DataGridColumnDef<TData | Record<string, string>>
    >
  }
...
function PivotGridDrilldownDialog<TData>({
  ...
  const drilldownRows = controller.drilldownState.cell
    ? controller.getDrilldownRows(controller.drilldownState.cell)
    : []
  ...
  const { table: drilldownTable, ...drilldownGridProps } = useDataGrid({
    data:
      drilldownRows.length > 0
        ? drilldownRows
        : ([{ __empty__: 'No rows' }] as Array<TData | Record<string, string>>),
  ...
  return (
    ...
    >
      <AwesomeDialogHeader
        title={
          controller.drilldownState.cell
            ? controller.getDrilldownTitle(controller.drilldownState.cell)
            : 'Drilldown'
        }
        description={
          controller.drilldownState.cell
            ? `${controller.drilldownState.cell.rawRowCount} contributing row(s)`
            : undefined
        }
      />
...
function PivotGridHeader<TData>({ controller }: PivotGridViewProps<TData>) {
  const rowHeaderColumns = controller.rowHeaderColumnIds
    ...
    .filter((column): column is Column<PivotGridRenderedRow<TData>> =>
      Boolean(column),
    )
          ...
          >
            ...
            {headerRow.cells.map((cell, cellIndex) => {
              const leafColumns = cell.columnIds
                .map((columnId) => controller.table.getColumn(columnId))
                .filter(
                  (column): column is Column<PivotGridRenderedRow<TData>> =>
                    Boolean(column),
                )
              ...
              const nextLeafColumn = nextLeafId
                ? controller.table.getColumn(nextLeafId)
                : undefined
              const firstHeader = firstColumn
                ? headerByColumnId.get(firstColumn.id)
                : undefined
              const showEndBorder = lastColumn
                ? getColumnBorderVisibility({
                    column: lastColumn,
...
function PivotGridRowHeaderCell<TData>({
  ...
  return (
    <div className="flex min-w-0 items-center gap-2">
      ...
      <span className={cn('truncate', summaryClassName)}>
        {headerCell.label}
      </span>
...
function PivotGridValueCell<TData>({
  ...
  return (
    ...
    >
      <span className={cn('truncate text-right', isSummary && 'font-medium')}>
        {content}
      </span>
      {cell ? (
        <Button
...
export function PivotGridView<TData>({
  controller,
}: PivotGridViewProps<TData>) {
  const rowHeaderColumns = controller.rowHeaderColumnIds
    .map((columnId) => controller.table.getColumn(columnId))
    .filter((column): column is Column<PivotGridRenderedRow<TData>> =>
      Boolean(column),
  ...
  return (
    <>
      <div
        className={cn('relative flex w-full flex-col', controller.className)}
      >
        <div
          ...
          style={{
            ...(controller.height === 'auto'
              ? { height: '100%' }
              : { maxHeight: `${controller.height}px` }),
          }}
        >
                          ...
                          ...getColumnPinningStyle({
                            ...
                            background:
                              row.kind === 'grand-total'
                                ? 'var(--muted)'
                                : row.kind === 'subtotal'
                                  ? 'color-mix(in oklab, var(--muted) 35%, white)'
                                  : row.kind === 'group'
                                    ? 'color-mix(in oklab, var(--muted) 55%, white)'
                                    : 'var(--background)',
                          }),
                          width: column.getSize(),
                        }}
