...
export function usePivotGrid<TData>(
  ...
  const valueColumnsById = useMemo(
    () =>
      new Map(
        viewModel.visibleLeafColumns.map((column) => [column.id, column]),
      ),
    ...
    () => [
      ...rowDimensions.map((dimension, index) => ({
        ...
        accessorFn: (row: PivotGridRenderedRow<TData>) =>
          row.headerCells[index]?.label ?? '',
        header: dimension.label,
        size:
          index === 0
            ? DEFAULT_ROW_HEADER_WIDTH
            : DEFAULT_CHILD_ROW_HEADER_WIDTH,
        minSize: 120,
        maxSize: 360,
        enableHiding: false,
        enableResizing: true,
  ...
  useEffect(() => {
    ...
    evaluatePivotGridCellColorRules(compiledColorRules, visibleCells).then(
      (nextMap) => {
        if (!cancelled) {
    ...
    (
      ...
      setColumnPinning((previousState) => {
        ...
        const nextLeft = (previousState.left ?? []).filter(
          (columnId) =>
            !rowHeaderSet.has(columnId) && !requestedSet.has(columnId),
        )
        const nextRight = (previousState.right ?? []).filter(
  ...
  const getDrilldownRows = useCallback(
    (cell: PivotGridRenderedCell<TData>) =>
      cell.rawRowIndices
        .map((rawRowIndex) => structure.data[rawRowIndex])
  ...
  const getDrilldownTitle = useCallback(
    (cell: PivotGridRenderedCell<TData>) => {
      ...
      const rowLabel =
        cell.rowPath.filter(Boolean).join(' / ') || GRAND_TOTAL_LABEL
      const columnLabel =
        cell.columnPath.filter(Boolean).join(' / ') || GRAND_TOTAL_LABEL
    ...
    (): PivotGridExportSnapshot<TData> => ({
      ...
      valueColumns: valueColumnIds.map((columnId) => {
        ...
        return {
          ...
          width:
            table.getColumn(columnId)?.getSize() ?? DEFAULT_VALUE_COLUMN_WIDTH,
  ...
  const drilldownColumns = useMemo(() => {
    ...
    const rows = drilldownState.cell
      ? getDrilldownRows(drilldownState.cell)
      : []
