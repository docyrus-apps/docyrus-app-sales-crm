...
import { type PatternStyle } from '@/lib/docyrus/pattern-styles';
...
export interface AwesomeDialogProps {
  ...
  minimizable?: boolean;
  /** Prevent closing when clicking or pressing outside the dialog */
...
export const MODAL_SIZES: Record<DialogSize, { width: string; maxHeight: string }> = {
...
export const SHEET_SIZES: Record<DialogSize, { horizontal: string; vertical: string; verticalWidth: string }> = {
...
export const RESIZE_CONSTRAINTS: Record<DialogSize, { min: number; max: number }> = {
  sm: { min: 320, max: 600 },
