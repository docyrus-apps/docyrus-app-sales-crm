  ...
  (
    ...
    const makeConnector = (completed: boolean) => {
      if (connector && isValidElement(connector)) {
        return cloneElement(connector, { _completed: completed } as Record<
          string,
          unknown
        >)
      }
    ...
    return (
      <StepperContext value={contextValue}>
        <div
          ...
          role="group"
          aria-label={t('ui.stepper.progress', 'Progress')}
          {...props}
        >
              ...
              <div
                ...
                className={cn(
                  ...
                  orientation === 'horizontal'
                    ? cn(
                        'flex-1 items-start',
                        alternativeLabel && 'flex-col items-center',
                      )
                    : 'flex-col',
                )}
              >
                ...
                >
                  ...
                  {conn &&
                    orientation === 'horizontal' &&
                    !alternativeLabel && (
                      <div className="mx-2 flex-1">{conn}</div>
                    )}
                </div>
                {conn && orientation === 'horizontal' && alternativeLabel && (
                  <div className="mt-2 w-full px-4">{conn}</div>
                )}
                {conn && orientation === 'vertical' && (
                  ...
                  >
                    <div className="flex gap-3">
                      ...
                      {step.props.children && (
                        <div className="flex-1 pb-4">{step.props.children}</div>
                      )}
                    </div>
                  </div>
                )}
