      ...
      <div
        ...
        {isDashed && (
          <div
            className={cn(
              'absolute inset-0',
              isVertical ? 'border-l-2 border-dashed border-muted-foreground/30' : 'border-t-2 border-dashed border-muted-foreground/30'
            )} />
        )}
        {animated && (
          <motion.div
            className={cn(
              ...
              isDashed && (isVertical ? 'border-l-2 border-solid border-primary bg-transparent' : 'border-t-2 border-solid border-primary bg-transparent')
            )}
            initial={false}
            animate={isVertical ? { height: _completed ? '100%' : 0 } : { width: _completed ? '100%' : 0 }}
