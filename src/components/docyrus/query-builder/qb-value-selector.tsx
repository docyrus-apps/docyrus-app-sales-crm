...
const QBValueSelector = memo(({
  ...
  const val = multiple ? Array.isArray(value) ? value.map(String).join(',') : String(value ?? '') : String(value ?? '');
  ...
  if (multiple) {
    ...
    const handleMultiChange = (newVal: string) => {
      const updated = selectedValues.includes(newVal) ? selectedValues.filter(v => v !== newVal) : [...selectedValues, newVal];
      ...
      <Select
        ...
        <SelectTrigger
          ...
          <SelectValue placeholder={title}>
            {selectedValues.length > 0 ? `${selectedValues.length} selected` : title}
          </SelectValue>
        </SelectTrigger>
        <SelectContent>
          {flatOptions.map(opt => (
            <SelectItem key={opt.name} value={opt.name}>
              {selectedValues.includes(opt.name) ? `✓ ${opt.label}` : opt.label}
            </SelectItem>
          ))}
        </SelectContent>
      </Select>
