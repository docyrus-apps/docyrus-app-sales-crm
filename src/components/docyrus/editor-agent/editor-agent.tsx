...
export function EditorAgent({
  ...
  const { messages, sendMessage, setMessages, status, stop, addToolResult } =
    useChat({
      transport,
  ...
  const handleSendMessage = useCallback(
    async ({ text, files }: AgentMessagePayload) => {
      ...
      const filePaths = hasFiles
        ? (await resolveFilePaths(files, trimmed)).filePaths
        : []

      sendOptionsRef.current = { filePaths, supportFiles: hasFiles }

      const sendOptions = hasFiles
        ? { body: { files: filePaths, supportFiles: true } }
        : undefined
  ...
  const handleSelectThread = useCallback(
    ...
    [activeThreadId, threads, setMessages, stop],
  )
