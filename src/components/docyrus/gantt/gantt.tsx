...
import {
  DndContext, MouseSensor, useDraggable, useSensor
} from '@dnd-kit/core';
...
/*
 * ---------------------------------------------------------------------------
 * Internal hooks (replacing @uidotdev/usehooks and jotai)
 * ---------------------------------------------------------------------------
 */

type MouseState = { x: number; y: number };

function useMouse<T extends HTMLElement>(): [MouseState, RefObject<T | null>] {
...
function useWindowScroll(): [{ x: number | null; y: number | null }] {
...
/*
 * ---------------------------------------------------------------------------
 * Shared state context (replaces jotai atoms)
 * ---------------------------------------------------------------------------
 */

type GanttSharedState = {
  dragging: boolean;
...
const GanttSharedContext = createContext<GanttSharedState>({
  dragging: false,
  setDragging: () => {},
  scrollX: 0,
  setScrollX: () => {}
...
const useGanttDragging = (): [boolean, (v: boolean) => void] => {
  const ctx = useContext(GanttSharedContext);

  return [ctx.dragging, ctx.setDragging];
};

const useGanttScrollX = (): [number, (v: number) => void] => {
  const ctx = useContext(GanttSharedContext);

  return [ctx.scrollX, ctx.setScrollX];
};
...
/*
 * ---------------------------------------------------------------------------
 * Headers (Daily / Monthly / Quarterly)
 * ---------------------------------------------------------------------------
 */
...
const DailyHeader: FC = () => {
  ...
  return gantt.timelineData.map(year => year.quarters
    ...
    .map((month, index) => (
      <div className="relative flex flex-col" key={`${year.year}-${index}`}>
        ...
        <GanttColumns
          columns={month.days}
          isColumnSecondary={(item: number) => [0, 6].includes(
...
const QuarterlyHeader: FC = () => {
  ...
  return gantt.timelineData.map(year => year.quarters.map((quarter, quarterIndex) => (
    <div
...
export const GanttFeatureItem: FC<GanttFeatureItemProps> = ({
  ...
  const handleItemDragMove = useCallback(() => {
    ...
    const delta
      = gantt.range === 'daily' ? getDifferenceIn(gantt.range)(currentDate, originalDate) : getInnerDifferenceIn(gantt.range)(currentDate, originalDate);
...
export const GanttFeatureRow: FC<GanttFeatureRowProps> = ({
  ...
  const subRowEndTimes: Array<Date> = [];
...
export const GanttProvider: FC<GanttProviderProps> = ({
  ...
  const scrollToFeature = useCallback(
    (feature: GanttFeature) => {
      ...
      const featureOffset = getOffset(feature.startAt, timelineStartDate, {
        zoom,
        range,
        columnWidth,
        sidebarWidth,
      ...
      const targetScrollLeft = Math.max(0, featureOffset);
  ...
  const sharedState = useMemo(
    () => ({
      dragging, setDragging, scrollX, setScrollX
    }),
    [dragging, scrollX]
  );

  return (
    <GanttSharedContext.Provider value={sharedState}>
