...
function computeExpandedPosition(
  ...
  switch (direction) {
    ...
    case 'rt':
      return { x: rect.left, y: rect.top + rect.height - targetH }

    case 'lb':
      return { x: rect.left + rect.width - targetW, y: rect.top }

    case 'lt':
      return {
        x: rect.left + rect.width - targetW,
        y: rect.top + rect.height - targetH,
      }
  }
}
...
function AwesomeCard({
  ...
  useEffect(() => {
    ...
    if (isExpanded) {
      ...
      const targetPos = computeExpandedPosition(
        rect,
        expandDirection,
        targetW,
        targetH,
      ...
      if (prefersReducedMotion) {
        gsap.set(card, {
          clearProps:
            'position,left,top,width,height,zIndex,boxShadow,overflow',
        })
  ...
  return (
    ...
    >
      ...
      <div
        ...
        onMouseEnter={
          expandable && expandTrigger === 'hover' ? handleMouseEnter : undefined
        }
        onMouseLeave={
          expandable && expandTrigger === 'hover' ? handleMouseLeave : undefined
        }
      >
        {children}
      </div>
...
function AwesomeCardTitle({
  ...
  return (
    <div
      ...
      className={cn(
        'text-base font-normal text-muted-foreground group-hover:text-foreground',
        className,
      )}
    >
      {children}
      ...
      <div className="overflow-hidden">
        <div className="mt-2 px-0 pb-0">
          <div
            ...
            className={cn('rounded-lg border bg-card px-5 py-4', className)}
          >
            {children}
          </div>
        </div>
...
function AwesomeCardExpandedBody({
  ...
  return (
    <div
      ...
      className={cn(
        'absolute inset-x-0 bottom-0 top-[2rem] mt-2 rounded-lg border bg-card px-5 py-4 overflow-auto',
        className,
      )}
      style={{ opacity: 0 }}
    >
