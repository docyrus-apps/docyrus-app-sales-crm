...
function buildMetadataPills(
  ...
  switch (type) {
    case 'call': {
      ...
      if (m.direction) {
        pills.push({
          ...
          icon: m.direction === 'inbound' ? ArrowDownLeftIcon : ArrowUpRightIcon
        });
      }
      if (m.duration) {
    ...
    case 'meeting': {
      ...
      if (m.attendee_count) pills.push({ text: `${m.attendee_count} attendees`, icon: UserIcon });
    ...
    case 'status_update':
      return (m.from_status || m.to_status) ? [{ text: `${m.from_status ?? '?'} \u2192 ${m.to_status ?? '?'}` }] : [];

    case 'task': {
      const pills: Array<MetadataPill> = [];

      if (m.priority) pills.push({ text: `${m.priority}` });
      if (m.due_date) pills.push({ text: `Due: ${m.due_date}`, icon: ClockIcon });
      if (m.assigned_to_name) pills.push({ text: `${m.assigned_to_name}`, icon: UserIcon });

      return pills;
    }

    case 'chat':
      return m.channel_name ? [{ text: `#${m.channel_name}`, icon: HashIcon }] : [];
