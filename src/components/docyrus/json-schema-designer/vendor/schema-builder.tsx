...
// @ts-nocheck
/* eslint-disable */
/* eslint-disable no-restricted-syntax -- vendored extend-hq schema-builder; preserve original style */
...
import {
  type LucideIcon,
  Brackets as SecondBracketIcon,
  Check as TextCheckIcon,
  ChevronDown,
  Code2 as SourceCodeSquareIcon,
  GripVertical as DragDropVerticalIcon,
  Hash as InputNumericIcon,
  List as LeftToRightListBulletIcon,
  Plus as Add01Icon,
  Table as TableIcon,
  Type as InputTextIcon,
  X as CancelCircleIcon,
} from 'lucide-react'
...
/**
 * Renders one of the icons we re-mapped to lucide-react. The original extend-hq
 * source uses `<HugeiconsIcon icon={X}>` everywhere; this thin component
 * preserves that call-site shape so the rest of the file changes minimally.
 */
...
/*
 * The original extend-hq source uses `@pierre/diffs` for a virtualized syntax-
 * highlighted JSON view. The dependency pulls a web worker + WASM bundle that
 * adds ~MB-sized vendor chunks for a single read-only code block. We drop it
 * here and render JSON as a plain `<pre>` inside a shadcn `ScrollArea` — no
 * syntax highlighting, but no extra dependencies either. Theme is read from
 * `useDocyTheme()` so the panel still respects the dark / light flag.
 */
...
function useResolvedCodeThemeType(
...
export function SchemaJsonView({
  ...
  /*
   * `theme` + `scrollResetKey` retained on the public API for source-compat
   * with the original component but no longer needed once the @pierre/diffs
   * highlighter is gone — we just render the formatted JSON in a `<pre>`.
   */
  void useResolvedCodeThemeType(theme)
  void scrollResetKey
  const contents = React.useMemo(
    () => formatJson(serializeSchema(schema)),
    [schema],
  )

  return (
    <div className="relative m-0 h-full overflow-hidden rounded-none bg-muted/30 text-foreground">
      <ScrollAreaVirtualizer
        className="h-full min-w-0"
        contentClassName="min-w-full"
      >
        <pre className="block min-w-full whitespace-pre overflow-auto px-4 py-3 font-mono text-[12.8px] leading-relaxed">
          {contents}
        </pre>
      </ScrollAreaVirtualizer>
    </div>
