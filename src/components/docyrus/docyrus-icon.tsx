...
function parseIcon(icon: string, defaultLib: string = 'kv'): ParsedIcon {
  if (icon === 'dot') {
    return {
      lib: null, group: null, name: 'dot', isEmoji: false, isDot: true
    };
  }

  if (isEmoji(icon)) {
    return {
      lib: null, group: null, name: icon, isEmoji: true, isDot: false
    };
  }
...
export interface DocyrusIconProps extends VariantProps<typeof docyrusIconVariants> {
  icon: string;
  lib?: string;
  group?: string | null;
  className?: string;
...
export const DocyrusIcon = memo(({
  ...
  if (parsed.isDot) {
    return (
      <span className={cn(variantClass, 'items-center justify-center', className)}>
        <span>&#x2022;</span>
      </span>
    );
  }
  ...
  const loader = (
    <span
      aria-hidden="true"
      className={cn(variantClass, 'rounded-sm bg-current opacity-10', className)} />
  );
