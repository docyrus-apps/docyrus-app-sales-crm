...
export function LogActivityForm({
  ...
  const [internalType, setInternalType] =
    useState<LoggableActivityType>(defaultActivityType)
  ...
  const resolvedPlaceholder =
    placeholderProp ?? DEFAULT_PLACEHOLDERS[activeType]
  ...
  const handleSubmit = useCallback(() => {
    ...
    sectionRef?.reset()
    setIsEmpty(true)
  }, [disabled, isSubmitting, editor, activeType, getSectionRef, onSubmit])
  ...
  const contextValue = useMemo<LogActivityFormContextValue>(
    ...
    [mentionUsers, emailAttachments, onAttach, onRemoveAttachment],
  )
  ...
  return (
    <LogActivityFormProvider value={contextValue}>
      ...
      >
        {/* Tab bar */}
        <div className="flex items-center gap-0 overflow-x-auto border-b px-3">
          {LOGGABLE_ACTIVITY_TYPES.map((type) => {
            ...
            return (
              ...
              >
                ...
                {isActive && (
                  <motion.div
                    layoutId="log-activity-type-underline"
                ...
                >
                  <div className="flex flex-col gap-2 px-3 pt-3 pb-2">
                    ...
                    <EditorContainer variant="comment">
                      <Editor
                        variant="comment"
                        className="min-h-[60px] grow text-sm"
                        disabled={disabled}
                        onKeyDown={(e) => {
