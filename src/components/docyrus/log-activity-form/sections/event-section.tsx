...
import {
  CalendarIcon, Check, ChevronsUpDown, Plus
} from 'lucide-react';
                  ...
                  <div className="flex p-2 sm:flex-col">
                    {Array.from({ length: 24 }, (_, i) => i).map(hour => (
                      <Button
                        key={hour}
                        size="icon"
                        variant={startValue.getHours() === hour ? 'default' : 'ghost'}
                  ...
                  <div className="flex p-2 sm:flex-col">
                    {Array.from({ length: 12 }, (_, i) => i * 5).map(minute => (
                      <Button
                        key={minute}
                        size="icon"
                        variant={startValue.getMinutes() === minute ? 'default' : 'ghost'}
                        className="aspect-square shrink-0 sm:w-full"
                        onClick={() => handleTimeChange('start', 'minute', minute)}>
                    ...
                    {Array.from({ length: 24 }, (_, i) => i)
                      ...
                      .map(hour => (
                        <Button
                          key={hour}
                          size="icon"
                          variant={endValue.getHours() === hour ? 'default' : 'ghost'}
                          className="aspect-square shrink-0 sm:w-full"
                ...
                <ScrollArea className="w-64 sm:w-auto">
                  <div className="flex p-2 sm:flex-col">
                    {Array.from({ length: 12 }, (_, i) => i * 5)
                      .filter(minute => endSelectedHour !== startHour || minute >= startMinute)
                      .map(minute => (
                        <Button
                          key={minute}
                          size="icon"
                          variant={endValue.getMinutes() === minute ? 'default' : 'ghost'}
                          className="aspect-square shrink-0 sm:w-full"
                          onClick={() => handleTimeChange('end', 'minute', minute)}>
                          {minute.toString().padStart(2, '0')}
                        </Button>
                      ))}
                  </div>
          ...
          <div className="flex items-center gap-1 rounded-lg border p-0.5">
            <button
              ...
              className={cn(
                ...
                mode === 'select' ? 'bg-accent text-accent-foreground' : 'text-muted-foreground hover:text-foreground'
              )}>
              Select existing
            </button>
            <button
              ...
              className={cn(
                'flex-1 rounded-md px-3 py-1 text-xs font-medium transition-colors',
                mode === 'create' ? 'bg-accent text-accent-foreground' : 'text-muted-foreground hover:text-foreground'
              )}>
