...
function ApprovalSummary({
  ...
  const actorName = getApprovalUserName(respondedByUser ?? null, respondedBy ?? null);
        ...
        <div className="text-muted-foreground flex flex-wrap items-center gap-1.5 text-xs">
          {respondedAt && (
            <span className="inline-flex items-center gap-1">
              ...
              {formatApprovalTimestamp(respondedAt)}
            </span>
          )}
  ...
  return (
    <div className="flex flex-col divide-y rounded-md border bg-muted/20">
      {steps.map((step, idx) => {
        ...
        return (
          <div
            key={step.id ?? `${step.status ?? 'step'}-${step.respondedAt ?? idx}`}
...
export function ApprovalStatusValue({
  value,
  className
}: DocyrusValueProps) {
