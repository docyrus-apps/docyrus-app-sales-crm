...
import {
  type ChangeEvent, useCallback, useMemo, useRef, useState
} from 'react';
...
interface PostComposerProps {
  ...
  onSubmit: (content: Value, parentId?: string, files?: Array<File>, entities?: Array<LinkedEntity>) => void;
  isPending: boolean;
  parentId?: string;
  placeholder?: string;
  compact?: boolean;
          ...
          <div className="mt-1.5 flex flex-wrap gap-1.5">
            {linkedEntities.map(entity => (
              <span
                ...
                <span className="max-w-30 truncate">{entity.display_value}</span>
                <button
                  type="button"
...
function PendingFileList({
  ...
  const previews = useMemo(
    () => files.map(file => isImageFile(file.type) ? URL.createObjectURL(file) : null),
    [files]
  );
