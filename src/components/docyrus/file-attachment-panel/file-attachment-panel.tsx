...
export function FileAttachmentPanel({
  ...
  const oneDrive = useOneDrivePicker(oneDriveConfig);
  const googleDrive = useGoogleDrivePicker(googleDriveConfig);
  ...
  const processUpload = useCallback(
    async (file: File) => {
      ...
      try {
        ...
        setUploadingFiles(prev => prev.map(uf => uf.id === uploadId ? { ...uf, progress: 100, status: 'complete' } : uf));
        setTimeout(() => {
          setUploadingFiles(prev => prev.filter(uf => uf.id !== uploadId));
        }, 1500);
      } catch {
        setUploadingFiles(prev => prev.map(uf => uf.id === uploadId ? { ...uf, status: 'error', error: 'Upload failed' } : uf));
      }
    },
    [onUploadFile]
  );
  ...
  const renderLoading = () => (
    <div className="flex h-full min-h-50 flex-col items-center justify-center gap-4 px-1 text-sm text-muted-foreground">
