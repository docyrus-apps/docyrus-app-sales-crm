      ...
      <Popover open={open} onOpenChange={setOpen}>
        ...
        >
          <div className="flex flex-col gap-1">
            <h4 id={labelId} className="font-medium leading-none">
              {draftSorting.length > 0
                ? t('ui.dataGrid.sortBy', 'Sort by')
                : t('ui.dataGrid.noSortingApplied', 'No sorting applied')}
            </h4>
            <p
              id={descriptionId}
              className={cn(
            ...
            >
              {draftSorting.length > 0
                ? t(
                    'ui.dataGrid.modifySorting',
                    'Modify sorting to organize your rows.',
                  )
                : t(
                    'ui.dataGrid.addSorting',
          ...
          <div className="flex w-full items-center gap-2">
            ...
            >
              {t('ui.dataGrid.addSort', 'Add sort')}
            </Button>
            {draftSorting.length > 0 && (
              <Button
                variant="outline"
              ...
              >
                {t('ui.dataGrid.resetSorting', 'Reset sorting')}
              </Button>
            )}
            <Button size="sm" className="ml-auto rounded" onClick={onSortApply}>
              {t('ui.dataGrid.applySort', 'Apply sort')}
            </Button>
          </div>
        </PopoverContent>
      </Popover>
      ...
      >
        <Popover open={showFieldSelector} onOpenChange={setShowFieldSelector}>
          ...
          >
            <Command>
              <CommandInput
                placeholder={t('ui.dataGrid.searchFields', 'Search fields...')}
              />
              <CommandList>
                <CommandEmpty>
                  {t('ui.dataGrid.noFieldsFound', 'No fields found.')}
                </CommandEmpty>
                <CommandGroup>
                  {columns.map((column) => (
                    <CommandItem
          ...
          >
            {SORT_ORDER_VALUES.map((value) => (
              <SelectItem key={value} value={value}>
                {value === 'asc'
                  ? t('ui.dataGrid.sortOrderAsc', 'Asc')
                  : t('ui.dataGrid.sortOrderDesc', 'Desc')}
              </SelectItem>
            ))}
          </SelectContent>
        </Select>
