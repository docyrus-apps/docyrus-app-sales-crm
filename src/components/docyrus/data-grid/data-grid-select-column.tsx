...
import {
  useCallback, useId, type ComponentProps, type MouseEvent, type ReactNode
} from 'react';
...
function DataGridSelectCell<TData>({
  ...
  const rowNumber = enableRowMarkers ? (meta?.getVisualRowIndex?.(row.id) ?? row.index + 1) : undefined;
