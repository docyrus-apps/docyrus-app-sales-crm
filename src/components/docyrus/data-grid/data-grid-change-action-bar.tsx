...
import {
  useCallback, useMemo, useState, type RefObject
} from 'react';
...
export function DataGridChangeActionBar<TData>({
  ...
  const changes = useMemo(() => {
    ...
    for (const [rowId, entry] of changeMapRef.current) {
      ...
      const rowLabel = getRowLabel ? getRowLabel(entry.originalRow, entry.dataIndex) : `Row ${entry.dataIndex + 1}`;
      ...
      <Popover open={popoverOpen} onOpenChange={setPopoverOpen}>
        <PopoverTrigger asChild>
          <button
            ...
            {changedRowCount}{' '}
            {changedRowCount === 1 ? 'item' : 'items'} changed
          </button>
        </PopoverTrigger>
        <PopoverContent
          side="top"
