import {
  type CSSProperties, type ComponentType, type ReactNode, type RefObject, type SVGProps
} from 'react';
...
export function getColumnPinningStyle<TData>(params: {
  ...
  const {
    column, dir = 'ltr', withBorder = false, background = 'var(--background)'
  } = params;
  ...
  return {
    boxShadow: withBorder ? isLastLeftPinnedColumn ? isRtl ? '4px 0 4px -4px var(--border) inset' : '-4px 0 4px -4px var(--border) inset' : isFirstRightPinnedColumn ? isRtl ? '-4px 0 4px -4px var(--border) inset' : '4px 0 4px -4px var(--border) inset' : undefined : undefined,
...
export function scrollCellIntoView<TData>(params: {
  ...
  const {
    container, targetCell, tableRef, direction, viewportOffset, isRtl
  }
    = params;
  ...
  const viewportLeft = isActuallyRtl ? containerRect.left + rightPinnedWidth + viewportOffset : containerRect.left + leftPinnedWidth + viewportOffset;
  const viewportRight = isActuallyRtl ? containerRect.right - leftPinnedWidth - viewportOffset : containerRect.right - rightPinnedWidth - viewportOffset;
