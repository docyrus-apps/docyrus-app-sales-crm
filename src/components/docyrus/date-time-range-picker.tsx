...
>(
  (
    ...
    function handleDateSelect(date: Date | undefined) {
      ...
      const newEnd = defaultDuration
        ? applyDuration(newStart)
        : currentEnd
          ? new Date(currentEnd)
          : new Date(date)
    ...
    return (
      <div
        ref={ref}
        className={cn(dateTimeRangePickerVariants({ size }), className)}
        {...props}
      >
                  ...
                  <div className="flex sm:h-67 sm:flex-row">
                    <ScrollArea className="w-64 sm:w-auto">
                      <div className="flex p-2 sm:flex-col">
                        {Array.from({ length: hourCount }, (_, i) => i).map(
                          (hour) => (
                            <Button
                              key={hour}
                              size="icon"
                        ...
                        {Array.from({ length: 12 }, (_, i) => i * 5).map(
                          (minute) => (
                            <Button
                              ...
                              size="icon"
                              variant={
                                startValue && startValue.getMinutes() === minute
                                  ? 'default'
                                  : 'ghost'
                              }
                              className="aspect-square shrink-0 sm:w-full"
                        ...
                        {Array.from({ length: 12 }, (_, i) => i * 5)
                          ...
                          .map((minute) => (
                            <Button
                              ...
                              size="icon"
                              variant={
                                endValue && endValue.getMinutes() === minute
                                  ? 'default'
                                  : 'ghost'
                              }
                              className="aspect-square shrink-0 sm:w-full"
