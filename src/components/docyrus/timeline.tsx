...
const timelineLineVariants = cva('absolute left-1/2 w-0.5 -translate-x-1/2', {
  variants: {
    lineStyle: {
      solid: '',
...
function StatusDot({
  ...
  const dotSize =
    status === 'pending' ? Math.round(config.statusDot * 0.6) : config.statusDot
...
function TimelineRow({
  ...
  const lineVariant = resolveLineStyle(
    item.status,
    item.lineStyle,
    globalLineStyle,
  )
  const isRight =
    layout === 'left' || (layout === 'alternate' && index % 2 === 0)
  ...
  const dot = item.status ? (
    ...
    <div
      ...
      style={{
        ...(item.dotColor && variant === 'default'
          ? { backgroundColor: item.dotColor }
          : {}),
        ...(item.dotColor && variant === 'outline'
  ...
  const line = !isLast && (
    <div
      className={cn(
        ...
        lineVariant === 'solid'
          ? getLineColorClass(item.status)
          : getDashBorderColorClass(item.status),
      )}
    ...
    rowContent = (
      <>
        <div className={cn('flex flex-1 justify-end', isRight && 'invisible')}>
          <div className="flex-1 pb-4 pr-3">{wrappedContent}</div>
        </div>
        <div
          className="relative flex flex-col items-center"
          style={{ width: dotColWidth }}
        >
          {dot}
          {line}
        </div>
        <div className={cn('flex flex-1', !isRight && 'invisible')}>
          <div className="flex-1 pb-4 pl-3">{wrappedContent}</div>
        </div>
      </>
    )
  } else {
  ...
  return <div className="flex items-stretch">{rowContent}</div>
}
  ...
  (
    ...
    return (
      <div ref={ref} className={cn('relative', className)} {...props}>
        {animated ? (
          <AnimatePresence mode="sync">{rows}</AnimatePresence>
        ) : (
          rows
        )}
      </div>
