...
export interface AvatarThumbnailProps extends VariantProps<
  typeof avatarThumbnailVariants
> {
  size?: number
  icon?: string | null
  color?: string | null
...
const AvatarThumbnail = forwardRef<HTMLDivElement, AvatarThumbnailProps>(
  ({ size = 8, icon, color, image, shape, className }, ref) => {
    ...
    return (
      <div
        ref={ref}
        className={cn(avatarThumbnailVariants({ shape }), sizeClass, className)}
        style={
          backgroundColor && !image?.signed_url
            ? { backgroundColor }
            : undefined
        }
      >
...
export { AvatarThumbnail, avatarThumbnailVariants }
