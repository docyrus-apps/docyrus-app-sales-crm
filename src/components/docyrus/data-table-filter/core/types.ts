...
export interface ColumnOption {
  ...
  label: string
  /* An optional dimmer secondary label displayed under the primary label. */
  secondaryLabel?: string
  /* The internal value of the option. */
  value: string
  /* An optional icon to display next to the label. */
  icon?: ReactElement | ReactElementType
  /* Optional image URL — rendered as a small avatar before the label. */
  imageUrl?: string
  /**
   * Optional accent color for the option (hex, hsl, etc). Used by enum /
   * status / select-style filter pickers to render a color dot before the
   * label so the picker matches the chip styling shown in table cells.
   */
  color?: string
}
...
/*
 * Async option loader for `option` / `multiOption` columns. When set on a
 * column, the filter UI replaces its static option list with a debounced
 * remote search + paginated load. Useful for relation / user-select fields
 * where the option set is too large to ship up front.
 */
export interface AsyncOptionsConfig {
  load: (params: {
    ...
    pageSize: number
    /**
     * Aborted when the controller unmounts or fires a fresh load (e.g.
     * search changes mid-flight). Implementations should forward this to
     * `fetch` / their HTTP client so cancelled requests don't keep loading
     * — important under React StrictMode dev where effects double-fire.
     */
    signal?: AbortSignal
  }) => Promise<{ items: Array<ColumnOption>; hasMore?: boolean }>
  /** Items per page. Default `25`. */
  pageSize?: number
  /** Debounce in ms before firing a new search. Default `300`. */
  debounceMs?: number
}
...
> = {
  ...
  icon: LucideIcon
  type: TType
  options?: TType extends OptionBasedColumnDataType
    ? Array<ColumnOption>
    : never
  ...
  /**
   * Async option loader. When present, the option list is fetched
   * (debounced search + paginated) instead of read from the static
   * `options` array. Only valid for `option` / `multiOption` types.
   */
  asyncOptions?: TType extends OptionBasedColumnDataType
    ? AsyncOptionsConfig
    : never
  /**
   * Per-column label for the `true` value in the boolean filter picker.
   * Falls back to the i18n `true` string when omitted.
   */
...
export type FilterOperatorDetails<
  TOperatorValue,
  T extends ColumnDataType,
> = FilterOperatorDetailsBase<TOperatorValue, T> &
  (
