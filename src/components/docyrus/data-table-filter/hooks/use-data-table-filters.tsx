...
import {
  type Dispatch, type SetStateAction, useMemo, useState
} from 'react';
...
>({
  ...
  if (
    ...
    const msg = 'If using controlled state, you must specify both filters and onFiltersChange.';
    ...
    () => ({
      addFilterValue<TData, TType extends OptionBasedColumnDataType>(
        ...
        if (column.type === 'option') {
          setFilters((prev) => {
            ...
            return prev.map(f => f.columnId === column.id ? {
              columnId: column.id,
              type: column.type,
      ...
      setFilterValue<TData, TType extends ColumnDataType>(
        ...
        setFilters((prev) => {
          ...
          const isColumnFiltered = filter && filter.values.length > 0;
          const newValues
            = column.type === 'number' ? createNumberFilterValue(values as Array<number>) : column.type === 'date' ? createDateFilterValue(
  ...
  return {
    columns, filters, actions, strategy
