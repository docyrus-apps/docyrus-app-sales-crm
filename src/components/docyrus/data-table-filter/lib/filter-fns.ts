...
export function dateFilterFn(
  ...
  const d1 = filterVals[0] as Date
  const d2 = filterVals[1] as Date | undefined
...
export function numberFilterFn(
  ...
  const value = inputData
  const filterVal = filterValue.values[0] as number
  ...
  switch (filterValue.operator) {
    ...
    case 'is between': {
      const lowerBound = filterValue.values[0] as number
      const upperBound = filterValue.values[1] as number
