...
export function FilterValueDateDisplay<TData>({
  ...
  const formattedRangeStr = formatDateRange(filter.values[0] as Date, filter.values[1] as Date);
...
const OptionItem = memo(({
  ...
  const {
    value, label, icon: Icon, selected, count
  } = option;
...
export function FilterValueOptionController<TData>({
  ...
  useEffect(() => {
    setOptions(prev => prev.map(o => ({ ...o, selected: filter?.values.includes(o.value) ?? false })));
  }, [filter?.values]);
...
export function FilterValueDateController<TData>({
  ...
  function changeDateRange(value: DateRange | undefined) {
    ...
    const end
      = start && value && value.to && !isEqual(start, value.to) ? value.to : undefined;
  ...
  const changeType = useCallback(
    (type: 'single' | 'range') => {
      ...
      if (type === 'single')
        ...
        newValues
          = value - minMax[0] < minMax[1] - value ? createNumberRange([value, minMax[1]]) : createNumberRange([minMax[0], value]);
      }
