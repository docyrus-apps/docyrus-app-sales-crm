...
export function CalendarProvider({
  ...
  const updateSettings = useCallback((newPartialSettings: Partial<CalendarSettings>) => {
    setSettings({ ...settings, ...newPartialSettings });
  }, [setSettings, settings]);

  const setBadgeVariant = useCallback((variant: 'dot' | 'colored') => {
    setBadgeVariantState(variant);
    updateSettings({ badgeVariant: variant });
  }, [updateSettings]);

  const setView = useCallback((newView: TCalendarView) => {
  ...
  const setAgendaModeGroupBy = useCallback((groupBy: 'date' | 'color') => {
    setAgendaModeGroupByState(groupBy);
    updateSettings({ agendaModeGroupBy: groupBy });
  }, [updateSettings]);

  const filterEventsBySelectedColors = useCallback((color: TEventColor) => {
    const isColorSelected = selectedColors.includes(color);
    const newColors = isColorSelected ? selectedColors.filter(c => c !== color) : [...selectedColors, color];

    if (newColors.length > 0) {
      const filtered = allEvents.filter(event => newColors.includes(event.color));
  ...
  const filterEventsBySelectedUser = useCallback((userId: IUser['id'] | 'all') => {
