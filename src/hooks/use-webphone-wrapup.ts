  ...
  const submitWrapup = useCallback(
    async ({ callRecordId, phone, direction, input }: SubmitWrapupArgs) => {
      const activityPayload = buildCallActivityPayload({
        ...
        phone,
        direction,
        resolveActivityEnum: (field, token) =>
          resolveEnum('call_activity', field, token),
      })
      ...
      await calls
        ...
        .catch((error) =>
          console.warn('[Webphone] Call wrap-up patch failed', error),
        )

      invalidateCalls()
    },
    [activities, calls, resolveEnum, invalidateCalls],
  )
