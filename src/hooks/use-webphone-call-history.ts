...
export function useCustomerCallHistory(args: {
  ...
  return useQuery({
    queryKey: ['callcenter', 'customer-call-history', args.contactId ?? args.leadId ?? args.phone ?? 'none'],
    enabled:
...
limit
    ...
    queryFn: async () => (await calls.list({
        columns: CALL_HISTORY_COLUMNS,
        orderBy: 'created_on desc',
        limit
      })) as unknown as Array<CallHistoryRow>
...
export function useCustomerPinnedNotes(args: {
  ...
  return useQuery({
    queryKey: ['callcenter', 'pinned-notes', args.contactId ?? args.leadId ?? 'none'],
