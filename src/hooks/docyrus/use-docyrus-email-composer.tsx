...
export type DocyrusEmailProvider
  = 'aws' | 'smtp' | 'mailgun' | 'resend' | 'microsoft-graph' | 'gmail';
...
export interface UseDocyrusEmailComposerResult {
  ...
  send: (overrides?: DocyrusEmailSendOverrides) => Promise<DocyrusEmailSendResult>;
  isSending: boolean;
...
export function useDocyrusEmailComposer(
  ...
  const accountsQuery = useQuery<DocyrusEmailAccount[]>({
    ...
    queryFn: async () => {
      const response = await client.get<ListAccountsResponse | DocyrusEmailAccount[]>(listAccountsEndpoint);
  ...
  const resolvedSelectedAccountId = useMemo(() => {
    ...
    const firstAccessible = accounts.find(account => account.isUserAccessible !== false) ?? accounts[0];
  ...
  const [lastSendResult, setLastSendResult] = useState<DocyrusEmailSendResult | null>(null);
  ...
  const sendMutation = useMutation<DocyrusEmailSendResult, Error, DocyrusEmailSendOverrides | undefined>({
    mutationFn: async (overrides) => {
      ...
      if (typeof finalSendAsUser === 'boolean') payload.sendAsUser = finalSendAsUser;
      if (finalAttachments && finalAttachments.length > 0) payload.attachments = finalAttachments;

      const endpoint = sendEndpoint.replace('{accountId}', encodeURIComponent(accountId));
      const response = await client.post<SendEmailResponse | DocyrusEmailSendResult>(endpoint, payload);

      if (response && typeof response === 'object' && 'data' in response && (response as SendEmailResponse).data) {
  ...
  const handleSubjectChange = useCallback((next: string) => setSubject(next), []);
  ...
  const send = useCallback(
    (overrides?: DocyrusEmailSendOverrides) => sendMutation.mutateAsync(overrides),
    [sendMutation]
  );

  const refetchAccounts = useCallback(
    () => queryClient.invalidateQueries({ queryKey: ['docyrus-email-accounts', listAccountsEndpoint] }),
