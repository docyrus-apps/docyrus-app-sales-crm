...
export type DocyrusAgentThreadsClientParams = Record<
  string,
...
export interface DocyrusAgentThreadsClient {
  get: <T = unknown>(
    url: string,
    params?: DocyrusAgentThreadsClientParams,
  ) => Promise<T>
...
export function useDocyrusAgentThreads({
  ...
  const queryKey = useMemo(
    () =>
      [...THREADS_QUERY_KEY, agentId, deploymentId, projectId, userId] as const,
    [agentId, deploymentId, projectId, userId],
  )
  ...
  const listQuery = useQuery<Array<DocyrusAgentThread>, Error>({
    ...
    queryFn: async () => {
      ...
      const agentRule = deploymentId
        ? {
            field: 'tenant_ai_agent_deployment_id',
            operator: '=',
            value: deploymentId,
          }
        : { field: 'tenant_ai_agent_id', operator: '=', value: agentId }
      ...
      if (userId) {
        rules.push({
          combinator: 'or',
          rules: [
            { field: 'record_owner', operator: '=', value: userId },
            { field: 'record_owner', operator: 'shared_to_me' },
          ],
        })
      }
      ...
      const response = await client.get<unknown>(
        ...
        {
          columns:
            'id,subject,created_on,last_modified_on,body_text,tenant_ai_agent_id,tenant_ai_project_id,created_by',
          expand: 'created_by',
  ...
  const renameMutation = useMutation({
    mutationFn: async ({
      ...
      await client.patch(
        `/v1/apps/base/data-sources/thread/items/${threadId}`,
        { subject },
      )
    },
    onSuccess: () => {
      ...
      const response = await client.get<unknown>(
        '/v1/apps/base/data-sources/message/items',
        {
          ...
          filters: JSON.stringify({
            combinator: 'and',
            rules: [
              { field: 'thread', operator: '=', value: threadId },
              { field: 'archived', operator: '=', value: false },
            ],
          }),
...
function resolveCreatedByName(value: unknown): string | undefined {
  ...
  const composed = [obj.firstname, obj.lastname]
    .filter(Boolean)
    .join(' ')
    .trim()
  ...
  return rows
    ...
    .map((row) => {
      const role: UIMessage['role'] =
        row.role ?? (row.sender_name === 'Assistant' ? 'assistant' : 'user')
