...
export type DocyrusAgentInfoClientParams = Record<
  string,
...
export interface DocyrusAgentInfoClient {
  get: <T = unknown>(
    url: string,
    params?: DocyrusAgentInfoClientParams,
  ) => Promise<T>
}
...
export function useDocyrusAgentInfo({
  ...
  const query = useQuery<DocyrusAgentDeploymentEnvelope | null, Error>({
    ...
    queryFn: async () => {
      ...
      const path = deploymentId
        ? `/v1/ai/agent-deployments/${deploymentId}`
        : `/v1/ai/agent-deployments/base/${agentId}`
  ...
  const result = useMemo(() => {
    ...
    const empty = {
      agent: null as AgentProfile | null,
    ...
    const agent: AgentProfile = {
      ...
      welcomeMessage:
        envelope.welcomeMessage ?? source.welcomeMessage ?? undefined,
...
function resolveCapabilities(
  ...
  source:
    | NonNullable<DocyrusAgentDeploymentEnvelope['agent']>
    | DocyrusAgentDeploymentEnvelope,
): AgentCapabilities {
  ...
  const promptOptimizationChoice = (obj.promptOptimizationChoice ??
    envObj.promptOptimizationChoice) as AgentCapabilities['promptOptimizationChoice']

  return {
    supportFiles: pickBool('supportFiles'),
    supportWebSearch: pickBool('supportWebSearch'),
    supportDocumentSearch:
      pickBool('documentSearch') ?? pickBool('supportDocumentSearch'),
    supportThinking: pickBool('supportThinking'),
    supportDeepResearch: pickBool('supportDeepResearch'),
    supportWorkCanvas:
      pickBool('featureWorks') ?? pickBool('supportWorkCanvas'),
...
function unwrapEnvelope(
  response: unknown,
): DocyrusAgentDeploymentEnvelope | null {
  if (!response || typeof response !== 'object') return null

  const obj = response as { data?: unknown } & DocyrusAgentDeploymentEnvelope

  if (obj.data && typeof obj.data === 'object')
    return obj.data as DocyrusAgentDeploymentEnvelope
  ...
  if (Array.isArray(value)) {
    return value
      .map((item) =>
        typeof item === 'string'
          ? item
          : ((item as { text?: string; label?: string; prompt?: string })
              ?.text ??
